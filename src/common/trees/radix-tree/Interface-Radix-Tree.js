...
class InterfaceRadixTree extends InterfaceTree {
  ...
  /**
     ...
     */
  add (input, value) {
    if (!Buffer.isBuffer(input)) { input = WebDollarCryptoData.createWebDollarCryptoData(input).buffer }

    if (input.length === 0) { throw { message: 'No input' } }
    ...
    while (i < input.length) {
      ...
      while (skipOptimization && childFound === false && nodeCurrent !== null) { //
          ...
          if (match !== null) { // we found  a match in the edge
            ...
            if (match.length < nodeCurrent.edges[j].label.length) {
              ...
              if (i === input.length) { // the prefix became a solution
                if (nodeCurrent.value !== null) { throw { message: 'the node already includes a value....' } } else { nodeCurrent._setNodeValue(value) }
        ...
        if (nodePrevious !== nodeCurrent) { skipOptimization = true } else skipOptimization = false
      }
  ...
  delete (input) {
    ...
    if (!searchResult.node.isLeaf()) { throw { message: "couldn't delete because input is not a leaf node" } }
