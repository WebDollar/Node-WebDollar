...
class InterfaceAccountantRadixTree extends InterfaceRadixTree {
  ...
  _checkInvalidNode (node) {
    ...
    if (node.isBalancesValid() && node.value.balances < 0) { return false }
  ...
  validateAccount (node) {
    ...
    if (node.isSumValid() === false) { return false } else { initialSum = node.sum }

    this._computeAccount(node)

    if (node.isSumValid() === false) { return false }
    if ((initialSum < node.sum) === false) { return false } // different sum
  ...
  _computeAccount (node) {
    if (node === null || node === undefined) { throw { message: "Couldn't compute the Sum because Node is empty" } }

    let sum

    if (node.isBalancesValid()) { sum = node.value.balances } else { sum = 0 }

    if (node.edges.length > 0) {
      for (let i = 0; i < node.edges.length; i++) {
        if (node.edges[i].targetNode.isSumValid() === false) { sum += this._computeAccount(node.edges[i].targetNode) } else { sum += node.edges[i].targetNode.sum }
      }
    }
  ...
  refreshAccount (node, forced) {
    ...
    if (!result) {
      ...
      if (node.parent !== null) {
        if (!this.validateAccount(node.parent)) { result = result && this.refreshAccount(node.parent, true) }
      }
    }
