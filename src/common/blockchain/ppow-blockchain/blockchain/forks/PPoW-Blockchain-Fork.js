...
class PPoWBlockchainFork extends InterfaceBlockchainFork {
  ...
  async initializeFork () {
    if (this.blockchain.agent.light) {
      if (!(await this._downloadProof())) { return false }
    }
  ...
  async _downloadProof () {
    ...
    if (this.blockchain.agent.light) {
      ...
      for (let i = 0; i < this.sockets.length; i++) {
        ...
        if (!proofPiData) { BansList.addBan(socket, 30000, 'proofPiFailed') } else {
          this.sockets[0] = socket
          break
        }
      }

      if (!proofPiData) { throw { message: 'Proof Failed to answer' } }
      ...
      if (this.blockchain.forksAdministrator.findForkByProofs(proofPiData.hash) !== null) { throw { message: 'fork proof was already downloaded' } }
      ...
      while (downloading && pos < timeoutCount) {
        ...
        if (!Buffer.isBuffer(answer.data)) { throw { message: 'accountantTree data is not a buffer' } }
      ...
      if (proofsList.length < 150) { console.error('PROOFS LIST length is less than 150', proofsList.length) }
      ...
      if (!this.forkProofPi.validateProofLastElements(consts.POPOW_PARAMS.m)) { throw { message: 'Prof Pi is invalid' } }
  ...
  _validateFork (validateHashesAgain, firstValidation) {
    ...
    if (!this.blockchain.agent.light) { return InterfaceBlockchainFork.prototype._validateFork.call(this, validateHashesAgain) }

    if (!this.forkProofPi) throw { message: 'Proof is invalid being null' }

    if (this.blockchain.proofPi) {
      if (this._isProofBetter()) { return true }
    } else return true
  }
  ...
  preForkClone (cloneBlocks = true) {
    if (this.blockchain.agent.light) { this._forkProofPiClone = this.blockchain.proofPi }

    return InterfaceBlockchainFork.prototype.preForkClone.call(this, cloneBlocks)
  }

  revertFork () {
    if (this.blockchain.agent.light) { this.blockchain.proofPi = this._forkProofPiClone }
  }
  ...
  _shouldTakeNewProof () {
    if (!this.blockchain.proofPi) { return true }
