...
class PPoWHelper {
  ...
  LCA (proofs1, proofs2) {
    ...
    while (i1 >= 0 && i2 >= 0) {
      ...
      if (block1.hash.equals(block2.hash)) { return block1 } else if (block1.height > block2.height) { i1-- } else { i2-- }
    }
  ...
  _localGood (superLength, underlyingLength, miu) {
    ...
    // using big Number
    if (new BigNumber(superLength).isGreaterThan((new BigNumber(1).minus(consts.POPOW_PARAMS.d)).multipliedBy(new BigNumber(2).pow(-miu)).multipliedBy(underlyingLength))) { return true } else { return false }
  }
  ...
  _superchainQuality (underlyingChain, superChain, miu) {
    if (consts.POPOW_PARAMS.m < 1) { throw { message: 'superchainQuality is not good' } }

    if (superChain.blocks.length <= consts.POPOW_PARAMS.m) { return false }
    ...
    while (mP >= consts.POPOW_PARAMS.m && mP < underlyingChain.blocks.length) {
      if (!this._localGood(Math.min(upperChain.blocks.length, mP), Math.min(downChain.blocks.length, mP), miu)) { return false }
  ...
  _multilevelQuality (underlyingChain, superChain, miu) {
    ...
    for (let miu1 = miu; miu1 >= 1; miu1--) {
      ...
      if (upperChain.blocks.length >= consts.POPOW_PARAMS.k1) {
        if (!(CStar.blocksGreaterLevel(miu).blocks.length >= (1 - consts.POPOW_PARAMS.d) * new BigInteger(2).pow(miu - miu1) * upperChain.blocks.length)) { return false }
      }
    }
  ...
  good (underlyingChain, superChain, miu) {
    if (this._superchainQuality(underlyingChain, superChain, miu) === false) { return false }
