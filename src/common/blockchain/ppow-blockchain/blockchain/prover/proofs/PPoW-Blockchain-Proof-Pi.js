...
class PPowBlockchainProofPi extends PPoWBlockchainProofBasic {
  ...
  blocksGreaterLevel (miu) {
    ...
    for (let i = 0; i < this.blocks.length; ++i) {
      if (miu <= this.blockchain.prover.provesCalculated.allBlocks[ this.blocks[i] ]) { list.push(this.blocks[i]) }
    }
  ...
  blocksLessLevel (miu) {
    ...
    for (let i = 0; i < this.blocks.length; ++i) {
      if (this.blockchain.prover.provesCalculated.allBlocks[ this.blocks[i] ] <= miu) { list.push(this.blocks[i]) }
    }
  ...
  downSuperChainGetUnderlyingChain (underlyingChain) {
    ...
    if (first === -1 || last === -1) { return new PPowBlockchainProofPi(this.blockchain, []) } else {
      ...
      for (let i = first; i <= last; i++) { newUnderlyingChain.blocks.push(underlyingChain.blocks[i]) }
  ...
  serializeProof (proof) {
    ...
    for (let i = 0; i < proof.interlinks.length; i++) {
      if (proof.interlinks[i] === 0) { interlinks.push(Serialization.serializeNumber1Byte(0)) } else {
    ...
    } else { buffer.push(Serialization.serializeNumber1Byte(containGemesis === true ? 1 : 0)) }
  ...
  deserializeProof (buffer, offset = 0) {
    ...
    while (interlinksLength !== currentInterlinkIterator) {
      ...
      if (currentInterlinkPrefix === 0) {
        ...
        if (containsGenesis && genesisPosition === currentInterlinkIterator) { deserializeHeight = -1 } else {
