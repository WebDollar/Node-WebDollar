...
class InterfaceBlockchainFork extends InterfaceBlockchainForkBasic {
  async includeForkBlock (block) {
    if (await this._validateForkBlock(block, block.height) === false) { throw { message: 'includeForkBlock failed for ', height: block.height } }
  ...
  async _validateForkBlock (block, height) {
    ...
    if (BlockchainGenesis.isPoSActivated(block.height)) { block.blockValidation.blockValidationType['skip-validation-PoW-hash'] = true }
  ...
  async validateForkImmutability () {
    ...
    if (this.blockchain.blocks.blocksStartingPoint < this.blockchain.blocks.length - consts.BLOCKCHAIN.FORKS.IMMUTABILITY_LENGTH) {
      ...
      // verify if there were only a few people mining in my last 30 blocks
        ...
        let addresses = {}; let addressesCount = 0
        ...
        for (let i = this.forkStartingHeight; i < this.blockchain.blocks.length; i++) {
          ...
          { throw { message: 'Validate for Immutability failed' } } else { return true } // there were just 3 miners, probably it is my own fork...
        }
      }
    }
  ...
  _showDebugInfo () {
    ...
    for (let forkBlock of this.forkBlocks) {
      for (let tx of forkBlock.data.transactions.transactions) { Log.log('Transaction', Log.LOG_TYPE.BLOCKCHAIN_FORKS, tx.toJSON()) }
  ...
  async saveFork () {
    ...
    if (this.blockchain) {
      success = await this.blockchain.semaphoreProcessing.processSempahoreCallback(async () => {
        ...
        if (await this._validateFork(false, false) === false) { return Log.error('validateFork was not passed', Log.LOG_TYPE.BLOCKCHAIN_FORKS) }

        if (await this.deleteAlreadyIncludedBlocks() === false) { return Log.error('deleteAlreadyIncludedBlocks blocks no longer exist', Log.LOG_TYPE.BLOCKCHAIN_FORKS) }

        if (global.TERMINATED) return false

        // mark the forkBlocks to avoid deletion
        for (let forkBlock of this.forkBlocks) { forkBlock.isForkBlock = true }
        ...
        // making a copy of the current blockchain
          ...
          for (let index = 0; index < this.forkBlocks.length; index++) {
            ...
            if (await this.saveIncludeBlock(index, revertActionsNewBlocks, false) === false) { throw ({ message: "fork couldn't be included in main Blockchain ", index: index }) }

            revertActionsNewBlocks.push({ name: 'block-added', height: forkBlock.height })

            if (!process.env.BROWSER && (!this.downloadBlocksSleep || (index > 0 && index % 10 !== 0))) { forkBlock.blockValidation.blockValidationType['skip-sleep'] = true } else { await this.blockchain.sleep(2) }

            if (forkBlock.height % 50 === 0) { Log.log('FORK PROCESSING: ' + index, Log.LOG_TYPE.BLOCKCHAIN_FORKS) }
          }
        ...
        for (let forkBlock of this.forkBlocks) { forkBlock.isForkBlock = false }
    ...
    if (this.downloadAllBlocks) { Blockchain.synchronizeBlockchain() }
  ...
  async postForkTransactions (forkedSuccessfully, oldBlocks) {
    ...
    if (forkedSuccessfully) {
      ...
      for (let block of oldBlocks) { await block.data.transactions.unconfirmTransactions() }

      for (let block of this.forkBlocks) { await block.data.transactions.confirmTransactions(block.height) }
    } else {
      for (let block of this.forkBlocks) { await block.data.transactions.unconfirmTransactions() }

      for (let block of oldBlocks) { await block.data.transactions.confirmTransactions(block.height) }
    }
  }
