...
class InterfaceBlockchainFork {
  ...
  async _validateFork (validateHashesAgain, firstValidation) {
    ...
    if (validateHashesAgain) {
      for (let i = 0; i < this.forkBlocks.length; i++) {
        if (!(await this._validateForkBlock(this.forkBlocks[i], this.forkStartingHeight + i))) { throw { message: 'validateForkBlock failed for ', index: i } }
      }
    }
  ...
  async _validateChainWork () {
    ...
    if (forkWork.lesser(chainWork)) { throw { message: 'forkWork is less than chainWork', forkWork: forkWork.toString(), chainWork: chainWork.toString() } }
  }
  ...
  _createBlockValidation_BlockchainValidation (height, forkHeight) {
    ...
    if (height !== this.forkChainLength - 1) { validationType['skip-calculating-proofs'] = true }
  ...
  async deleteAlreadyIncludedBlocks () {
    ...
    for (let i = 0; i < this.forkBlocks.length - 1; i++) {
      if (this.forkBlocks[i].height > this.blockchain.blocks.length - 1) { break }
  ...
  getSocket () {
    ...
    if (Array.isArray(socket)) { socket = socket[0] }
  ...
  _findSocket (socket) {
    for (let i = 0; i < this.sockets.length; i++) {
      if (this.sockets[i] === socket) { return i }
    }
  ...
  pushSocket (socket, priority) {
    if (this._findSocket(socket) === -1) {
      if (priority) { this.sockets.splice(0, 0, socket) } else {
        if (socket.latency !== 0) {
          for (let i = 0; i < this.sockets.length; i++) {
            if (this.sockets[i].latency > socket.latency) { return this.sockets.splice(i, 0, socket) }
          }
        }
