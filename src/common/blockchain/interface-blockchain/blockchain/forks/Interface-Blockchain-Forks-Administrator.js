...
class InterfaceBlockchainForksAdministrator {
    ...
    NodesList.emitter.on('nodes-list/disconnected', async (nodesListObject) => {
      for (let i = this.forks.length - 1; i >= 0; i--) {
        if (!this.forks[i].forkIsSaving) {
          ...
          if (this.forks[i].sockets.length === 0 && !this.forks[i].forkIsSaving) { this.forks.splice(i, 1) }
        }
      }
    })
  }
  ...
  findFork (socket, hash, forkProof) {
    ...
    if (forkFound) {
      if (Math.random() < 0.001) { console.error('discoverAndProcessFork - fork already found by forkLastBlockHeader') }
    ...
    if (forkFound) {
      if (Math.random() < 0.001) { console.error('discoverAndProcessFork - fork already found by socket') }
  ...
  createNewFork (sockets, forkStartingHeight, forkChainStartingPoint, forkChainLength, forkChainWork, forkChainHashes, ready) {
    ...
    for (let key in forkChainHashes) {
      if (this.findForkyByChainHash(key)) { return fork }
    }
      ...
      for (let j = 0; j < this.forks.length; j++) {
        for (let q = 0; q < this.forks[j].sockets.length; q++) {
          if (this.forks[j].sockets[q].node.sckAddress === sockets[i].node.sckAddress ||
                         this.forks[j].sockets[q].node.sckAddress.uuid === sockets[i].node.sckAddress.uuid) { return this.forks[j] }
        }
      }
    }
  ...
  findForkyByChainHash (header) {
    ...
    if (Buffer.isBuffer(header)) { header = header.toString('hex') }

    for (let fork of this.forks) { if (fork.forkChainHashes[header]) return fork }
  ...
  findForkByProofs (proof) {
    ...
    try {
      for (let i = 0; i < this.forks.length; i++) {
        if (this.forks[i].forkProofPi.equalsProofs(proof)) { return this.forks[i] }
      }
    } catch (exception) {
  ...
  deleteFork (fork) {
    ...
    for (let i = this.forks.length - 1; i >= 0; i--) {
      if (!this.forks[i] || this.forks[i] === fork || this.forks[i].forkId === fork) { this.forks.splice(i, 1) }
    }
