        ...
        return new Promise( async (resolve)=>{
            ...
            socket.node.on("api/subscribe/address/transactions/answer/"+address, async (data)=>{
                ...
                for (let k in data.transactions){
                    ...
                    let tx = null;
                    if (transaction ) tx = await Blockchain.Transactions._createTransaction( transaction.from, transaction.to, transaction.nonce, transaction.timeLock, transaction.version, transaction.extra, undefined);
