...
class NanoWalletProtocol {
  constructor () {
    ...
    if (this._allowConsensus) { this.initializeNanoProtocol() }
  }
  ...
  async initializeNanoProtocol () {
    for (let i = 0; i < NodesList.nodes.length; i++) { await this._initializeSocket(NodesList.nodes[i].socket) }
  ...
  async _initializeSocket (socket) {
    if (!this._allowConsensus && Blockchain.agent.consensus) { return true }
    ...
    let trySubscribe = async (trials) => {
      ...
      if (trials > 0) { return await trySubscribe(trials - 1) }
    }
  ...
  virtualizeWallet (socket) {
    ...
    for (let i = 0; i < Blockchain.Wallet.addresses.length; i++) { this.virtualizeAddress(Blockchain.Wallet.addresses[i].address, [socket]) }
  }
