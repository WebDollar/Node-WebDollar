...
class TransactionsDownloadManager {
      ...
      if (this.smallestTrial >= this._transactionsQueue[txId].fails) {
        ...
        if (currentTime - this._transactionsQueue[txId].lastTrialTime > 1000 * 10 || typeof this._transactionsQueue[txId].lastTrialTime === 'undefined') {
          ...
          if (this._transactionsQueue[txId].fails <= 2) {
            ...
            if (this._transactionsQueue[txId].socket !== undefined) { return { id: txId } } else { this.removeTransaction(txId) }
          } else { this.removeTransaction(txId) }
        }
      } else { foundHighFailedTransaction = true }
    }

    // Increase trials limit for processing next time or decrease it
    if (!foundPriorityTransaction) {
      if (!foundHighFailedTransaction) { this.smallestTrial = this.smallestTrial === 0 ? 0 : this.smallestTrial-- } else {
        this.smallestTrial++
  ...
  async addTransaction (socket, txId, topPriority) {
    ...
    if (Blockchain.blockchain.transactions.pendingQueue.findPendingTransaction(txId)) { return true }
    ...
    if (!this.findTransactionById(txId.toString('hex'))) {
      ...
      if (topPriority) {
        if (typeof this._transactionsQueue[txId] !== 'undefined') { this._transactionsQueue[txId].fails = 0 }
      }
  ...
  async _processSocket (socketID) {
    if (NodesList.isConsensus(this.blockchain.blocks.length)) {
      try {
        ...
        if (this._socketsQueueLength > 0) { await this.transactionsProtocol.downloadTransactions(this._socketsQueue[selectedSocket], 0, 40, consts.SETTINGS.MEM_POOL.MAXIMUM_TRANSACTIONS_TO_DOWNLOAD) }
      } catch (exception) {
        console.warn('Faield to process socket')
      }
    } else { setTimeout(this._processSocket.bind(this, socketID), 40 * 1000) }
  }
    ...
    if (NodesList.isConsensus(this.blockchain.blocks.length)) {
      for (let i = 0; i <= 20; i++) {
        try {
          ...
          if (typeof firstUneleted === 'object') { txId = firstUneleted.id }
          ...
          if (txId) {
            ...
            // Check socket
              ...
              // Try to download transaction by hash
                ...
                if (!this._transactionsQueue[txId]) { continue }
                ...
                if (Buffer.isBuffer(this._transactionsQueue[txId].buffer)) {
                  ...
                  if (socket) {
                    ...
                    if (this._transactionsQueue[txId]) {
                      if (this._socketsQueue[socket.node.sckAddress.uuid].downloadFails > 500) { this._unsubscribeSocket(socket) }
                    }
                  }
                }
              }
            ...
            if (!found && this._transactionsQueue[txId]) {
              ...
              if (this._transactionsQueue[txId].socket.length === this._transactionsQueue[txId].totalSocketsProcessed - 1) { this._transactionsQueue[txId].totalSocketsProcessed = 0 }
            } else if (wasAdded) {
              this.removeTransaction(txId)
            }
          }
  ...
  _increaseSocketPenalty (socket) {
    ...
    if (socket.node.protocol.transactionsDownloadingManager.penaltyPoints >= 20) { socket.node.protocol.transactionsDownloadingManager.penaltyDate = new Date().getTime() }
  }
    ...
    for (let txId in this._transactionsQueue) {
      if (this._transactionsQueue[txId]) {
        for (let i = this._transactionsQueue[txId].socket.length - 1; i >= 0; i--) {
          if (this._transactionsQueue[txId].socket[i].node.sckAddress.uuid === socket.node.sckAddress.uuid) { this._transactionsQueue[txId].socket.splice(i, 1) }

          if (this._transactionsQueue[txId].socket.length === 0) { this.removeTransaction(txId) }
        }
      } else { this.removeTransaction(txId) }
    }
