...
class InterfaceBlockchainTransactionFrom {
  ...
  setFrom (addresses, currencyTokenId) {
    ...
    for (let fromObject of addresses) {
      if (fromObject.unencodedAddress) {
        if (typeof fromObject.unencodedAddress === 'object' && fromObject.unencodedAddress.unencodedAddress) { fromObject.unencodedAddress = fromObject.unencodedAddress.unencodedAddress }
      ...
      if (typeof fromObject.publicKey === 'string') { fromObject.publicKey = new Buffer(fromObject.publicKey, 'hex') }

      if (typeof fromObject.signature === 'string') { fromObject.signature = new Buffer(fromObject.signature, 'hex') }

      if (!fromObject.signature) { fromObject.signature = new Buffer(consts.TRANSACTIONS.SIGNATURE_SCHNORR.LENGTH) }

      if (typeof fromObject.amount === 'string') { fromObject.amount = parseInt(fromObject.amount) }
    }

    for (let fromObject of addresses) {
      // optional
      if (!fromObject.unencodedAddress) { fromObject.unencodedAddress = InterfaceBlockchainAddressHelper._generateUnencodedAddressFromPublicKey(fromObject.publicKey, false) }
    }
  ...
  /**
     ...
     */
  validateFrom () {
    if (this.addresses.length === 0) { throw { message: 'From.addresses is empty', addresses: this.addresses } }

    if (this.addresses.length >= 256) throw { message: 'Too many inputs. Max 256' }

    if (!this.currencyTokenId || this.currencyTokenId === null) {
      throw {
    ...
    if (!Buffer.isBuffer(this.currencyTokenId)) { throw { message: 'To.currencyTokenId is not a buffer', currencyTokenId: this.currencyTokenId } }

    if (!(this.currencyTokenId.length === consts.MINI_BLOCKCHAIN.TOKENS.WEBD_TOKEN.LENGTH || this.currencyTokenId.length === consts.MINI_BLOCKCHAIN.TOKENS.OTHER_TOKENS.LENGTH)) { throw { message: 'To.currencyTokenId is not valid', currencyTokenId: this.currencyTokenId } }
    ...
    for (let fromObject of this.addresses) {
      ...
      if (!fromObject.publicKey) { throw { message: 'From.address.publicKey ' + index + ' is not specified', address: fromObject, index: index } }

      if (!fromObject.unencodedAddress) { fromObject.unencodedAddress = InterfaceBlockchainAddressHelper._generateUnencodedAddressFromPublicKey(fromObject.publicKey) }

      if (!fromObject.unencodedAddress) { throw { message: 'From.address.unencodedAddress is not specified', address: fromObject, index: index } }

      if (!InterfaceBlockchainAddressHelper.getUnencodedAddressFromWIF(fromObject.unencodedAddress)) { throw { message: 'From.address.unencodedAddress is not a valid address', address: fromObject, index: index } }

      if (!Buffer.isBuffer(fromObject.unencodedAddress) || fromObject.unencodedAddress.length !== consts.ADDRESSES.ADDRESS.LENGTH) { throw { message: 'From.address.unencodedAddress ' + index + ' is not a buffer', address: fromObject, index: index } }

      if (!Buffer.isBuffer(fromObject.publicKey) || fromObject.publicKey.length !== consts.ADDRESSES.PUBLIC_KEY.LENGTH) { throw { message: 'From.address.publicAddress ' + index + ' is not a buffer', address: fromObject, index: index } }

      if (!InterfaceBlockchainAddressHelper._generateUnencodedAddressFromPublicKey(fromObject.publicKey).equals(fromObject.unencodedAddress)) { throw { message: 'From.address.unencodedAddress ' + index + " doesn't match the publicKey", address: fromObject, index: index } }

      if (!WebDollarCoins.validateCoinsNumber(fromObject.amount)) { throw { message: 'From.Object Amount is not specified', amount: fromObject.amount, index: index } }

      if (fromObject.amount <= 0) { throw { message: 'Amount is an invalid number', amount: fromObject.amount, index: index } }
    }
  ...
  calculateInputSum () {
    // validate amount
    let inputValues = []; let inputSum = 0
  ...
  findAddressIndex (unencodedAddress) {
    ...
    for (let i = 0; i < this.addresses.length; i++) {
      if (BufferExtended.safeCompare(this.addresses[i].unencodedAddress, unencodedAddress)) { return i }
    }
  ...
  serializeForSigning (unencodedAddress) {
    ...
    if (typeof unencodedAddress === 'number') { position = unencodedAddress } else { position = this.findAddressIndex(unencodedAddress) }

    if (position < 0 || position > this.addresses.length) { throw { message: 'address was not found' } }
  ...
  validateSignatures () {
    ...
    for (let fromObject of this.addresses) {
      ...
      if (!fromObject.signature) { throw { message: 'From.address.signature is not specified', address: fromObject, index: index } }

      if (!Buffer.isBuffer(fromObject.signature) || fromObject.signature.length !== consts.TRANSACTIONS.SIGNATURE_SCHNORR.LENGTH) { throw { message: 'From.address.signature ' + index + ' is not a buffer', address: fromObject, index: index } }

      if (!ed25519.verify(fromObject.signature, this.serializeForSigning(index), fromObject.publicKey)) { throw { message: 'From.address.signature ' + index + ' is not correct', address: fromObject, index: index } }

      if (!InterfaceBlockchainAddressHelper._generateUnencodedAddressFromPublicKey(fromObject.publicKey).equals(fromObject.unencodedAddress)) { throw { message: 'From.address.publicKey ' + index + " doesn't match address", address: fromObject, index: index } }
    }
  ...
  serializeFrom () {
    ...
    for (let i = 0; i < this.addresses.length; i++) {
      if (this.transaction.version <= 0x01) { array.push(Serialization.serializeToFixedBuffer(consts.ADDRESSES.ADDRESS.LENGTH, this.addresses[i].unencodedAddress)) }
  ...
  deserializeFrom (buffer, offset) {
    ...
    try {
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (this.transaction.version > 0x01) { address.unencodedAddress = InterfaceBlockchainAddressHelper._generateUnencodedAddressFromPublicKey(address.publicKey, false) }
