...
class InterfaceBlockchainTransaction {
  ...
  constructor (blockchain, from, to, nonce, timeLock, version, txId, validateFrom = true, validateTo = true, validateNonce = true, validateTimeLock = true, validateVersion = true, validateTxId = true) {
    ...
    if (validateTimeLock) {
      if (!timeLock) { this.timeLock = blockchain.blocks.length - 1 } else { this.timeLock = timeLock }
    }
    ...
    try {
      if (!(from instanceof InterfaceBlockchainTransactionFrom)) { from = this._createTransactionFrom(from) }
    ...
    try {
      if (!(to instanceof InterfaceBlockchainTransactionTo)) { to = this._createTransactionTo(to) }
    ...
    if (validateNonce) {
      if (!nonce) { nonce = this._computeNonce() }
    }
    ...
    try {
      if (validateFrom) { this.from.validateFrom() }
    } catch (exception) {
    ...
    try {
      if (validateTo) { this.to.validateTo() }
    } catch (exception) {
    ...
    if (validateTxId) {
      if (!txId.equals(this.txId)) { throw { message: 'TxId is invalid' } }
    }
  }
  ...
  get txId () {
    if (!this._txId) { this._txId = this._computeTxId() }
  ...
  validateTransactionOnce (blockHeight, blockValidationType = {}) {
    ...
    if (!this.from) { throw { message: 'Transaction Validation Invalid: From was not specified', from: this.from } }

    if (!this.to) { throw { message: 'Transaction Validation Invalid: To was not specified', to: this.to } }
    ...
    if (!WebDollarCoins.validateCoinsNumber(inputSum)) { throw { message: 'Transaction inputSum is invalid', inputSum: inputSum } }

    if (!WebDollarCoins.validateCoinsNumber(outputSum)) { throw { message: 'Transaction outputSum is invalid', inputSum: outputSum } }

    if (inputSum < outputSum) { throw { message: 'Transaction Validation Input is smaller than Output', input: inputSum, output: outputSum } }

    if (!this.validateTransactionEveryTime(blockHeight, blockValidationType)) { return false }
  ...
  isTransactionOK (avoidValidatingSignature = false, showDebug = true, blockValidationType = {}) {
    if (!avoidValidatingSignature) { this.validateTransactionOnce(undefined, { 'skip-validation-transactions-from-values': true }) }
  ...
  serializeTransaction (rewrite = false) {
    ...
    if (!this._serializated || rewrite) { this._serializated = this._serializeTransaction() }
  ...
  deserializeTransaction (buffer, offset = 0, returnTransaction = false) {
    if (!Buffer.isBuffer(buffer)) { buffer = WebDollarCryptoData.createWebDollarCryptoData(buffer).buffer }
  ...
  toJSON (dontIncludeTxId) {
    ...
    if (!dontIncludeTxId) { result.txId = this.txId.toString('hex') }
  ...
  processTransaction (multiplicationFactor = 1, minerAddress, revertActions, showUpdate) {
    if (multiplicationFactor === 1) { // adding transaction
      ...
      if (!this._preProcessTransaction(multiplicationFactor, revertActions, showUpdate)) return false
    } else { throw { message: 'multiplicationFactor is illegal' } }
  ...
  _processTransactionFees () {
    ...
    if (!WebDollarCoins.validateCoinsNumber(diffInFees)) { throw { message: 'Fees are invalid' } }
  ...
  set confirmed (newValue) {
    if (this._confirmed !== newValue) { this._confirmed = newValue }
