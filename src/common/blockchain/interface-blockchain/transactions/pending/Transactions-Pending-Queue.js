...
class TransactionsPendingQueue {
    ...
    pendingQueueTxTimeLockValidation(transaction){
        if ( this.blockchain.blocks.length + consts.BLOCKCHAIN.FORKS.IMMUTABILITY_LENGTH > transaction.timeLock && this.blockchain.blocks.length - 2 < transaction.timeLock )
            return true;
        else
            return false;
    }
