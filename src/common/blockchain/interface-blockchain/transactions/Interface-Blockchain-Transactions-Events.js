...
class InterfaceBlockchainTransactionsEvents {
  ...
  async listTransactions (addressWIF, maxBlockCount = 50) {
    if (addressWIF === '' || addressWIF === undefined || addressWIF === null || addressWIF === '') { return [] }

    if (!Buffer.isBuffer(addressWIF)) { addressWIF = BufferExtended.fromBase(addressWIF) }
  ...
  subscribeTransactionsChanges (addressWIF, callback) {
    ...
    if (!Buffer.isBuffer(addressWIF) && typeof addressWIF === 'string') { addressWIF = BufferExtended.fromBase(addressWIF) }
  ...
  _searchAddressInTransaction (unencodedAddress, transaction) {
    for (let i = 0; i < transaction.from.addresses.length; i++) {
      if (BufferExtended.safeCompare(transaction.from.addresses[i].unencodedAddress, unencodedAddress)) { return true }
    }

    for (let i = 0; i < transaction.to.addresses.length; i++) {
      if (BufferExtended.safeCompare(transaction.to.addresses[i].unencodedAddress, unencodedAddress)) { return true }
    }
  ...
  _checkTransactionIsSubscribed (address) {
    ...
    if (Buffer.isBuffer(address)) { name = 'transactions/changes/' + BufferExtended.toBase(address) } else { name = address }
