...
class ProcessWorker {
  ...
  async start (path, filename) {
    ...
    if (this._child.exitCode !== null) { return false }
    ...
    if (this._timeoutValidation === undefined) { this._timeoutValidation = setTimeout(this._validateWork.bind(this), 1000) }
  ...
  async send (length, block, difficulty, start, end, batch) {
    ...
    for (let i = 0; i < block.length; i++) { data += block[i] + '  ' }

    for (let i = 0; i < difficulty.length; i++) { data += difficulty[i] + ' ' }
    ...
    if (this._lastData === undefined) { this._sendDataTimeout = setTimeout(this._writeWork.bind(this, data), 10) } else { this._nextData = this._lastData }
  }
  ...
  async _deleteFile (prefix = '') {
    if (await fs.existsSync(this._outputFilename + this.suffix + prefix) === false) { return }
  ...
  async _validateWork () {
    ...
    try {
      ...
      if (hash !== this._prevHash) {
        ...
        if (data.type === 'b' || data.type === 's') { data.h = this._count }
