...
class Workers {
  ...
  constructor (ibb) {
    ...
    if (consts.TERMINAL_WORKERS.TYPE === 'cpu') { // MULTI THREADING MINING
      ...
      if (this.worker_batch === 0) { this.worker_batch = 10 * this.worker_batch_thread * this.workers_max }
    ...
    if (!FS.existsSync(this._worker_path + this._worker_path_file_name)) { Log.error('Worker build is missing.', Log.LOG_TYPE.default) }
    ...
    if (!this.makeUnresponsiveThreadsTimeout) { this.makeUnresponsiveThreadsTimeout = setTimeout(this._makeUnresponsiveThreads.bind(this), 5000) }
  }
  ...
  haveSupport () {
    ...
    if (consts.TERMINAL_WORKERS.TYPE === 'cpu') {
      if (consts.TERMINAL_WORKERS.CPU_MAX === -1) { return false }

      // it needs at least 2
      if (this.workers_max <= 1) { return false }
    }
    return true
  }
  ...
  stopMining () {
    try {
      ...
      for (let i = 0; i < this.workers_list.length; i++) {
        if (this.workers_list[i] !== undefined && typeof this.workers_list[i].kill === 'function') { this.workers_list[i].kill('SIGINT') }
      }
    } catch (exception) {
  ...
  async run (start, end, loop_delay = 2) {
    ...
    this._current_max = (end) || this._abs_end
    ...
    if (this._loopTimeout === undefined) { this._loopTimeout = setTimeout(this._loop.bind(this, loop_delay), 1) }
  }
  ...
  async _initiateWorkers () {
    ...
    if (consts.TERMINAL_WORKERS.TYPE === 'cpu-cpp') { count = 1 }

    for (let index = this.workers_list.length; index < count; index++) { await this._initializeWorker(index) }

    return this
  }

  async _initializeWorker (index) {
    if (this.workers_list[index] && typeof this.workers_list[index].kill === 'function') { this.workers_list[index].kill('SIGINT') }
  ...
  async _createWorker (index) {
    ...
    worker.on('message', async (msg) => {
      ...
      if (msg.type === 's' || msg.type === 'b') {
        ...
        if (msg.h) { this.ibb._hashesPerSecond += parseInt(msg.h) }
      }
      ...
      if (msg.type === 's') {
        ...
        if (msg.blockId && parseInt(msg.blockId) !== (this.ibb.blockId || this.ibb.block.height)) { return false }
        ...
        if (consts.DEBUG && !Blockchain.MinerPoolManagement.minerPoolStarted) {
          if (await this._validateHash(hash, nonce) === false) { return false }
        }
      ...
      if (msg.type === 'b') {
        ...
        for (let i = 0, l = this.ibb.bestHash.length; i < l; i++) {
          if (bestHash[i] < this.ibb.bestHash[i]) {
            change = true
            break
          } else if (bestHash[i] > this.ibb.bestHash[i]) { break }
        }
        ...
        if (!this._working && this._current >= this._current_max) { this._stopAndResolve() }

        if (consts.DEBUG) { await this._validateHash(bestHash, parseInt(msg.bestNonce)) }
  ...
  async _validateHash (initialHash, nonce) {
    if (consts.TERMINAL_WORKERS.TYPE === 'cpu-cpp' || consts.TERMINAL_WORKERS.TYPE === 'gpu') {
      ...
      if (hash.equals(initialHash) === false) {
  ...
  async _loop (_delay = 1) {
    try {
      if (!this.ibb.started || this.ibb.resetForced || (this.ibb.reset && this.ibb.useResetConsensus)) {
        if (!this._finished) { this._stopAndResolve() }
      ...
      await this.workers_list.forEach(async (worker, index) => {
        ...
        if (worker._is_batching) { return false }

        if (this._final_batch) {
          if (Blockchain.MinerPoolManagement && Blockchain.MinerPoolManagement.minerPoolStarted) { this.ibb.resetForced = true }

          return false
        }

        // add only the rest
        if (this._current_max - this._current < this.worker_batch) { this._final_batch = this._current_max - this._current }
        ...
        if (consts.TERMINAL_WORKERS.TYPE === 'cpu') {
          ...
          if (await worker.send(this.block.length, this.block, this.difficulty, this._current, this._current + batch, this.worker_batch_thread) === false) { return false }
        }
      })
    } catch (exception) {
