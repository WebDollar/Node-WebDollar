...
class InterfaceBlockchainBlockData {
  ...
  async validateBlockData (blockHeight, blockValidation) {
    if (!Buffer.isBuffer(this.minerAddress) || this.minerAddress.length !== consts.ADDRESSES.ADDRESS.LENGTH) { throw { message: 'data.minerAddress is empty' } }

    if (!this.hashData || !Buffer.isBuffer(this.hashData)) { throw { message: 'hashData is empty' } }

    // validate hash
    if (!blockValidation.blockValidationType['skip-block-data-validation']) {
      let hashData = this.calculateHashBlockData()

      if (!BufferExtended.safeCompare(hashData, this.hashData)) { throw { message: 'block.data hashData is not right' } }
    }

    if (!blockValidation.blockValidationType['skip-block-data-transactions-validation']) {
      if (await this.transactions.validateTransactions(blockHeight, blockValidation.blockValidationType) === false) { throw { message: 'transactions failed to validate' } }
    }
  ...
  _computeBlockDataHeaderPrefix (onlyHeader = false) {
    if (!this.transactions.hashTransactions) { this.transactions.hashTransactions = this.transactions.calculateHashTransactions() }
  ...
  serializeData (onlyHeader = false) {
    if (!Buffer.isBuffer(this.hashData) || this.hashData.length !== 32) { this.computeHashBlockData() }
  ...
  _setMinerAddress (minerAddressWIF) {
    if (!Buffer.isBuffer(minerAddressWIF)) { minerAddressWIF = BufferExtended.fromBase(minerAddressWIF) }

    let result = InterfaceBlockchainAddressHelper.getUnencodedAddressFromWIF(minerAddressWIF)

    if (!result) { throw { message: 'Miner address is not a valid WIF', address: minerAddressWIF } }
