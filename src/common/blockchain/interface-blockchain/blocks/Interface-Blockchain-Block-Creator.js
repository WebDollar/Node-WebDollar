...
class InterfaceBlockchainBlockCreator {
  ...
  async createBlockNew (minerAddress, blockValidation, transactions) {
    if (!blockValidation) { blockValidation = this.blockchain.createBlockValidation() }

    try {
      if (!Buffer.isBuffer(minerAddress)) { minerAddress = BufferExtended.fromBase(minerAddress) }
    } catch (exception) {
      console.log('error creating a new block invalid minerAddress', minerAddress, exception)
    }

    let restArgs = []
    for (let i = 2; i < arguments.length; i++) { restArgs.push(arguments[i]) }
