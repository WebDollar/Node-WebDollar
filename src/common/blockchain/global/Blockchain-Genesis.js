...
class BlockchainGenesis {
  constructor () {
    ...
    if (!consts.DEBUG) { this.timeStampOffset = 1524742312 } // main net
    else { this.timeStampOffset = 1529344475 } // test net
  ...
  validateGenesis (block) {
    if (block.timeStamp.length !== this.timeStamp.length) { throw { message: "Timestamp doesn't match", timestamp: block.timeStamp } }

    if (block.timeStamp > 0x000FFFFF) { throw { message: 'Timestamp is too old ', timestamp: block.timeStamp } }

    if (block.timeStamp < 0) { throw { message: 'Timestamp is invalid', timeStamp: block.timeStamp } }
  }
  ...
  isPoSActivated (height) {
    ...
    if (height < consts.BLOCKCHAIN.HARD_FORKS.POS_ACTIVATION) { return false } else {
      // 0..19  pos
      // 20..29 pow
