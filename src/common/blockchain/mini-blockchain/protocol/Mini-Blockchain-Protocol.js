...
class MiniBlockchainProtocol extends inheritProtocol {
  _validateBlockchainHeader (data) {
    ...
    if (typeof data.header.data.hashAccountantTree === 'string') { data.header.data.hashAccountantTree = Serialization.fromBase(data.header.data.hashAccountantTree) } else { data.header.data.hashAccountantTree = new Buffer(data.header.data.hashAccountantTree) }
  }
