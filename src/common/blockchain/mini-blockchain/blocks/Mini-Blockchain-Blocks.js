...
class MiniBlockchainBlocks extends InterfaceBlockchainBlocks {
  async spliceBlocks (after, showUpdate = true, revertActions) {
    ...
    if (!revertActions) { revertActions = new RevertActions(this.blockchain) }
    ...
    if (after === 0) {
      ...
      for (let i = oldLength - 1; i >= after; i--) {
        ...
        // remove transactions
        for (let j = block.data.transactions.transactions.length - 1; j >= 0; j--) { block.data.transactions.transactions[j].processTransaction(-1, block.data.minerAddress, undefined, showUpdate) }
