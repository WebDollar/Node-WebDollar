...
class MiniBlockchainTransaction extends InterfaceBlockchainTransaction {
  ...
  _validateNonce (blockValidationType) {
    ...
    if (nonce < this.nonce) {
      if (blockValidationType !== undefined && blockValidationType['take-transactions-list-in-consideration'] !== undefined && blockValidationType['take-transactions-list-in-consideration'].validation) {
        ...
        if (transactionsList === undefined) { transactionsList = this.blockchain.transactions.pendingQueue.listArray }

        transactionsList.forEach((transaction) => {
          if (transaction.from.addresses[0].unencodedAddress.equals(this.from.addresses[0].unencodedAddress)) { foundNonce[ transaction.nonce ] = true }
        })

        for (let i = nonce; i < this.nonce; i++) {
          if (!foundNonce[i]) { throw { message: 'Nonce is not right 2', myNonce: this.nonce, nonce: nonce, txId: this.txId.toString('hex') } }
        }
    ...
    if (nonce !== this.nonce) { throw { message: 'Nonce is not right', myNonce: this.nonce, nonce: nonce, txId: this.txId.toString('hex') } }
    ...
    try {
      this.blockchain.transactions.pendingQueue.listArray.forEach((pendingTransaction) => {
        if (BufferExtended.safeCompare(pendingTransaction.from.addresses[0].unencodedAddress, this.from.addresses[0].unencodedAddress) && pendingTransaction.nonce >= nonce) {
          ...
          if (pendingTransaction.nonce - nonce <= 1) { nonce++ }
        }
      })
    } catch (exception) {
  ...
  _processTransactionFees (multiplicationFactor = 1, minerAddress = undefined, revertActions, showUpdate) {
    ...
    if (!WebDollarCoins.validateCoinsNumber(diffInFees)) { throw { message: 'diffInFees is not number', address: minerAddress } }

    if (diffInFees < 0) { throw { message: 'Accountant Tree is negative' } }
