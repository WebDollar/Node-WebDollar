...
class MiniBlockchainTransactionFrom extends InterfaceBlockchainTransactionFrom {
  validateFromEnoughMoney (blockValidationType) {
    ...
    if (blockValidationType !== undefined && blockValidationType['take-transactions-list-in-consideration'] !== undefined && blockValidationType['take-transactions-list-in-consideration'].validation) {
      ...
      if (transactionsList === undefined) { transactionsList = this.transaction.blockchain.transactions.pendingQueue.listArray }
      ...
      for (let i = 0; i < transactionsList.length; i++) {
        ...
        if (BufferExtended.safeCompare(this.transaction.txId, transaction.txId, blockValidationType['validate-fast-transactions'])) { return false } // transaction is not taken in consideration
    ...
    this.addresses.forEach((fromObject, index) => {
      ...
      if (blockValidationType !== undefined && blockValidationType['take-transactions-list-in-consideration'] !== undefined && blockValidationType['take-transactions-list-in-consideration'].validation) {
        ...
        if (amounts[addr] !== undefined) { value += amounts[addr] }
      }

      if (value < 0) throw { message: "Accountant Tree Input doesn't exist", unencodedAddress: fromObject.unencodedAddress }

      if (value < fromObject.amount) { throw { message: 'Value is Less than From.address.amount', address: fromObject, index: index } }
    })
  ...
  processTransactionFrom (multiplicationFactor = 1, revertActions, showUpdate) {
    for (let i = 0; i < this.addresses.length; i++) {
      if (!WebDollarCoins.validateCoinsNumber(this.addresses[i].amount)) { throw { message: 'amount is not number', address: this.addresses[i] } }
