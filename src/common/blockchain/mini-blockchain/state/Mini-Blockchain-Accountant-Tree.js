...
class MiniBlockchainAccountantTree extends MiniBlockchainAccountantTreeEvents {
  ...
  updateAccount (address, value, tokenId, revertActions, showUpdate = true) {
    ...
    if (tokenId.length === consts.MINI_BLOCKCHAIN.TOKENS.WEBD_TOKEN.LENGTH && tokenId[0] === consts.MINI_BLOCKCHAIN.TOKENS.WEBD_TOKEN.VALUE) {
      ...
      if (showUpdate) { this.emitter.emit('accountant-tree/root/total', this.root.total.toString()) }
    }
  ...
  updateAccountNonce (address, nonceChange, revertActions, showUpdate) {
    ...
    if (!node.isLeaf()) { throw { message: "couldn't updateAccountNonce because node is not leaf", address: address } }
  ...
  getBalance (address, tokenId) {
    ...
    if (!node.isLeaf()) { return null } // throw {message: "couldn't get the value because input is not a leaf node", address: address, tokenId: tokenId };
  ...
  getAccountNonce (address) {
    ...
    if (!node) { return null } // throw {message: "getAccounantNonce address not found", address: address, tokenId: tokenId };
  ...
  async loadMiniAccountant (buffer, offset, includeHashes, name = 'accountantTree', showUpdate = true) {
    try {
      ...
      if (showUpdate) { this.emitBalancesChanges() }
  ...
  calculateNodeCoins (tokenId, node) {
    ...
    for (let i = 0; i < node.edges.length; i++) {
      if (node.edges[i].targetNode) { sum += this.calculateNodeCoins(tokenId, node.edges[i].targetNode) }
    }
