...
if (consts.POPOW_PARAMS.ACTIVATED) { inheritBlockchain = PPoWBlockchain } else { inheritBlockchain = InterfaceBlockchain }
  ...
  async simulateNewBlock (block, revertAutomatically, revertActions, callback, showUpdate = true) {
    ...
    try {
      if (!block.blockValidation.blockValidationType['skip-mini-blockchain-simulation']) {
        ...
        if (await block.data.transactions.validateTransactions(block.height, block.blockValidation.blockValidationType) === false) { throw { message: 'Validate Transactions is wrong' } }
        ...
        if (!block.data.transactions.processBlockDataTransactions(block, +1, revertActions, showUpdate)) { throw { message: 'Process Block Data Transactions failed' } }
      }

      if (await callback() === false) { throw { message: "couldn't process the InterfaceBlockchain.prototype.includeBlockchainBlock" } }
    } catch (ex) {
