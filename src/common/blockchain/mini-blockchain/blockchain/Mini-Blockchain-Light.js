...
class MiniBlockchainLight extends MiniBlockchainAdvanced {
  ...
  async includeBlockchainBlock (block, resetMining, socketsAvoidBroadcast, saveBlock, revertActions, showUpdate) {
    if (!block.blockValidation.blockValidationType['skip-validation']) {
      ...
      if (saveBlock) { NodeBlockchainPropagation.propagateBlock(block, socketsAvoidBroadcast) }
    } else {
      if (!(await this.inheritBlockchain.prototype.includeBlockchainBlock.call(this, block, resetMining, 'all', saveBlock, revertActions))) { throw { message: 'Error Including Blockchain Light Block' } }
    }
  ...
  async _onBlockCreated (block, saveBlock) {
    ...
    if (!(await this._recalculateLightPrevs(block.height, block, this.getSerializedAccountantTree(block.height), this.getSerializedAccountantTree(block.height, true), saveBlock))) { throw { message: '_recalculateLightPrevs failed' } }
  ...
  async _recalculateLightPrevs (height, block, serialization, serializationGzip, save = true) {
    if (block === undefined || block === null) { block = BlockchainGenesis }
    ...
    try {
      if (this.agent.light && save) {
        if (this.lightPrevDifficultyTargets[ this.getSavingSafePosition(height) ] !== undefined) { await this._saveLightSettings(this.getSavingSafePosition(height)) }
      }
    } catch (exception) {
  ...
  async _saveLightSettings (diffIndex) {
    ...
    try {
      ...
      if (diffIndex === undefined) { diffIndex = this.getSavingSafePosition() }

      if (!(await this.db.save(this._blockchainFileName + '_LightSettings_diffIndex', diffIndex))) { throw { message: "Couldn't be saved _LightSettings_diffIndex", diffIndex } }

      if (this.lightPrevDifficultyTargets[diffIndex] === undefined) { throw { message: '_saveLightSettings is undefined ', diffIndex: diffIndex } }
      ...
      if (!(await this.accountantTree.saveMiniAccountant(true, undefined, treeSerialization))) { throw { message: 'saveMiniAccountant', diffIndex } }
  ...
  async _loadLightSettings (serializationAccountantTreeInitial) {
    ...
    if (diffIndex === null || diffIndex === undefined) { diffIndex = this.getSavingSafePosition(numBlocks - 1) }
  ...
  async saveBlockchainTerminated () {
    ...
    try {
      ...
      if (this.blocks.length === 0) { throw { message: 'Nothing to Save' } }

      await this._saveLightSettings()

      if (!(await this.inheritBlockchain.prototype.saveBlockchain.call(this, this.blocks.length - this.getSavingSafePosition()))) { throw { message: "couldn't save the blockchain" } }
    } catch (exception) {
  ...
  async _loadBlockchain () {
    ...
    try {
      ...
      if (!(await this.accountantTree.loadMiniAccountant(undefined, undefined, true))) { throw { message: 'Problem Loading Mini Accountant Tree Initial' } }
      ...
      if (!answer.result) { throw { message: "couldn't load the Light Settings" } }

      this._difficultyNotValidated = false

      if (!(await this.inheritBlockchain.prototype._loadBlockchain.call(this, answer.diffIndex - 1))) { throw { message: 'Problem loading the blockchain' } }
  ...
  _getLoadBlockchainValidationType (indexStart, i, numBlocks, onlyLastBlocks) {
    ...
    if (this.agent !== undefined && this.agent.light === true) {
      ...
      if (i - indexStart < numBlocks + consts.BLOCKCHAIN.TIMESTAMP.VALIDATION_NO_BLOCKS) { validationType['skip-validation-timestamp'] = true }

      if (!this._difficultyNotValidated) { validationType['skip-difficulty-recalculation'] = true }
  ...
  getBlock (height = this.blocks.length) {
    if (this.agent !== undefined && this.agent.light === true && height !== 0) {
      if (this.proofPi !== undefined) {
        ...
        if (proofPiBlock !== undefined && proofPiBlock !== null) { return proofPiBlock }
      }
    }
  ...
  getDifficultyTarget (height = this.blocks.length) {
    if (this.agent && this.agent.light === true && height !== 0) {
      if (this.lightPrevDifficultyTargets[height]) { return this.lightPrevDifficultyTargets[height] }
    }
