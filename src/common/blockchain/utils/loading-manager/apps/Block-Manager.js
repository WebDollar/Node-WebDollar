...
class BlockManager extends MemoryManager {
  ...
  async _loadData (height) {
    try {
      ...
      if (!this.blockchain.agent.light) { block.difficultyTargetPrev = await this.blockchain.getDifficultyTarget(height - 1) }
      ...
      if (await block.loadBlock() === false) { throw { message: 'no block to load was found' } }
