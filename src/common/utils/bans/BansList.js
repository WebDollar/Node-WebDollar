...
class BansList {
  ...
  _findBan (sckAddress) {
    for (let i = 0; i < this._bans.length; i++) {
      if (this._bans[i].sckAddress.matchAddress(sckAddress, { 'uuid': true })) { return i }
    }
  ...
  _removeEmptyBans () {
    for (let i = this._bans.length - 1; i >= 0; i--) {
      if (!this._bans[i].sckAddress || !this._bans[i].isBanned()) { this._bans.splice(i, 1) }
    }
  }

  listBans () {
    if (this._bans.length > 0) { console.info('BANNNNNNNNNNNNNNS') }
