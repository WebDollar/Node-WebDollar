...
class NodeBlockchainPropagation {
  constructor () {
    ...
    NodesList.emitter.on('nodes-list/disconnected', (nodeListObject) => {
      for (let i = this._socketsAlreadyBroadcast.length - 1; i >= 0; i--) {
        if (this._socketsAlreadyBroadcast[i] === nodeListObject.socket) { this._socketsAlreadyBroadcast.slice(i, 1) }
      }

      for (let i = this._socketsPropagating.length - 1; i >= 0; i--) {
        if (this._socketsPropagating[i] === nodeListObject.socket) { this._socketsPropagating.slice(i, 1) }
      }
    })
  ...
  propagateBlock (block, socketsAvoidBroadcast) {
    ...
    if (socketsAvoidBroadcast) {
      ...
      for (let i = 0; i < NodesList.nodes.length; i++) {
        if (Blockchain.isPoolActivated && ([NODES_CONSENSUS_TYPE.NODE_CONSENSUS_MINER_POOL, NODES_CONSENSUS_TYPE.NODE_CONSENSUS_SERVER_FOR_MINER].indexOf(NodesList.nodes[i].socket.node.protocol.nodeConsensusType) >= 0)) { socketsAvoidBroadcast.push(NodesList.nodes[i].socket) }
      }
  ...
  propagateLastBlockFast (suspendOtherBlocks = true) {
    ...
    for (let i = 0; i < NodesList.nodes.length; i++) {
      if (!Blockchain.isPoolActivated || ([NODES_CONSENSUS_TYPE.NODE_CONSENSUS_MINER_POOL, NODES_CONSENSUS_TYPE.NODE_CONSENSUS_SERVER_FOR_MINER].indexOf(NodesList.nodes[i].socket.node.protocol.nodeConsensusType) < 0)) { NodesList.nodes[i].socket.node.protocol.sendLastBlock() }
    }
  }
  ...
  processPropagation () {
    ...
    if (block && this._socketsPropagating.length < consts.SETTINGS.PARAMS.CONNECTIONS.SOCKETS_TO_PROPAGATE_NEW_BLOCK_TIP) {
      let list = []
      for (let i = 0; i < NodesList.nodes.length; i++) {
        if (!this._findSocket(NodesList.nodes[i].socket)) { list.push(NodesList.nodes[i].socket) }
      }
      ...
      while (list.length > 0 && this._socketsPropagating.length < consts.SETTINGS.PARAMS.CONNECTIONS.SOCKETS_TO_PROPAGATE_NEW_BLOCK_TIP) {
        ...
        setTimeout(() => {
          ...
          for (let i = this._socketsPropagating.length; i >= 0; i--) {
            if (this._socketsPropagating[i] === socket) { this._socketsPropagating.splice(i, 1) }
          }
        }, 100 + Math.random() * 200)
      }
    }
  ...
  _findSocket (socket) {
    for (let i = 0; i < this._socketsPropagating.length; i++) {
      if (this._socketsPropagating[i] === socket) { return true }
    }

    for (let i = 0; i < this._socketsAlreadyBroadcast.length; i++) {
      if (this._socketsAlreadyBroadcast[i] === socket) { return true }
    }
  ...
  _deleteDisconnectedSockets () {
    for (let i = this._socketsAlreadyBroadcast.length - 1; i >= 0; i--) {
      if (this._socketsAlreadyBroadcast[i].disconnected) { this._socketsAlreadyBroadcast.splice(i, 1) }
    }

    for (let i = this._socketsPropagating.length - 1; i >= 0; i--) {
      if (this._socketsPropagating[i].disconnected) { this._socketsPropagating.splice(i, 1) }
    }
  }
}
