...
class NodePropagationProtocol {
  ...
  async _processList (list, nodeType) {
    for (let key in list) {
      ...
      if (list[key].next && list[key].next > 0) { list[key].sock.node.sendRequest((nodeType === NODE_TYPE.NODE_TERMINAL ? 'propagation/request-all-wait-list/full-nodes' : 'propagation/request-all-wait-list/light-nodes'), { index: list[key].next, count: this.count(list[key].sock) }) }
  ...
  initializeSocketForPropagation (socket) {
    ...
    setTimeout(() => {
      if ([NODES_CONSENSUS_TYPE.NODE_CONSENSUS_MINER_POOL].indexOf(socket.node.protocol.nodeConsensusType) === 0) { socket.node.sendRequest('propagation/request-all-wait-list/full-nodes', { index: 0, count: this.count(socket) }) }
    ...
    try {
      ...
      switch (op) {
        ...
        case 'new-light-nodes':
          ...
          if (list.length > MAX_WAITLIST_QUEUE_LENGTH) { break }
          ...
          if (lastWaitlist !== undefined && response.next !== undefined && response.next > 0) { lastWaitlist.next = response.next }
  ...
  initializeNodesPropagation (socket) {
    ...
    socket.node.on('propagation/request-all-wait-list/full-nodes', response => {
      ...
      if (answer && answer.list.length > 0) { socket.node.sendRequest('propagation/nodes', { 'op': 'new-full-nodes', addresses: answer.list, next: answer.next }) }
    })
    ...
    socket.node.on('propagation/request-all-wait-list/light-nodes', response => {
      ...
      if (answer && answer.list.length > 0) { socket.node.sendRequest('propagation/nodes', { 'op': 'new-light-nodes', addresses: answer.list, next: answer.next }) }
    })
  ...
  _getWaitlist (response, list) {
    try {
      ...
      let answer = []
      for (let i = index * count; i < (index + 1) * count && i < list.length; i++) { answer.push(list[i].toJSON()) }
