...
class SocketExtend {
  extendSocket (socket, address, port, uuid, level) {
    ...
    socket.node.on = (name, callback) =>
      socket.on(name, async (data) => {
        ...
        try {
          ...
          if (consts.DEBUG) { console.log('socket.on raised an error', exception) }
        }
      })
    ...
    if (socket.webRTC) { socket.node.sendRequest = SocketProtocol.prototype.sendRequestWebPeer.bind(socket) } else { socket.node.sendRequest = SocketProtocol.prototype.sendRequestSocket.bind(socket) }
