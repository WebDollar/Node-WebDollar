...
class SocketProtocol {
  ...
  sendRequestWaitOnce (request, requestData, answerSuffix, timeOutInterval = 3000) {
    ...
    return new Promise((resolve) => {
      ...
      if (timeOutInterval) { timeoutId = setTimeout(clearReturnFunction, timeOutInterval) }

      let answer = this.node.sendRequest(request, requestData)

      if (!answer) { clearReturnFunction() }
    })
  }
    ...
    if (typeof data === 'object' && data !== null) {
      for (let prop in data) {
        if (data.hasOwnProperty(prop)) {
          if (isArrayBuffer(data[prop])) { data[prop] = Buffer.from(data[prop]) } else
          if (prop === 'type' && data.type === 'Buffer' && data.hasOwnProperty('data')) {
            data = new Buffer(data)
            return data
          } else { data[prop] = SocketProtocol._processBufferArray(data[prop]) }
        }
      }
    }
