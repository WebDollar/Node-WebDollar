...
class NodeSignalingServerProtocol {
  ...
  initializeSignalingServerService (socket) {
    ...
    socket.node.on('signals/server/connections/established-connection-was-dropped', (data) => {
      ...
      if (!connection) {
        ...
        if (waitlist) waitlist.acceptWebPeers = true
      }
    })
    ...
    socket.node.on('signals/server/connections/was-established-successfully', (data) => {
      if (!data.connectionId) {
        let connection = SignalingServerRoomList.searchSignalingServerRoomConnectionById(data.connectionId)
        if (connection) connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionEstablished
      }
    })
    ...
    socket.node.on('signals/server/connections/error-establishing-connection', (data) => {
      if (!data.connectionId) {
        let connection = SignalingServerRoomList.searchSignalingServerRoomConnectionById(data.connectionId)
        if (!connection) connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionError
      }
    })
  ...
  _initializeClient1 (client1) {
    ...
    client1.node.on('signals/client/initiator/generate-initiator-signal/answer', (initiatorAnswer) => {
      ...
      if (initiatorAnswer === null || initiatorAnswer.initiatorSignal === undefined) { connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionError } else if (initiatorAnswer.accepted === false && initiatorAnswer.message === 'Already connected') { connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionAlreadyConnected } else if (initiatorAnswer.accepted === false && initiatorAnswer.message === "I can't accept WebPeers anymore") { this._clientIsNotAcceptingAnymoreWebPeers(client1, connection) } else if (initiatorAnswer.accepted === true) {
    ...
    client1.on('signals/client/initiator/join-answer-signal/answer', (result) => {
      try {
        ...
        if (result === null || result === undefined) { connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionError } else if (result.established === false && result.message === 'Already connected') { connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionAlreadyConnected } else if (result.established === false && result.message === "I can't accept WebPeers anymore") { this._clientIsNotAcceptingAnymoreWebPeers(client1, connection) } else { connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionEstablished }
      } catch (exception) {
    ...
    client1.node.on('signals/client/initiator/receive-ice-candidate/answer', async (answer) => {
      ...
      if (answer === null || answer === undefined) { connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionError } else if (answer.established === false && answer.message === "I can't accept WebPeers anymore") { this._clientIsNotAcceptingAnymoreWebPeers(client1, connection) }
    })
  }
  ...
  _initializeClient2 (client2) {
    ...
    client2.node.on('signals/client/answer/receive-initiator-signal/answer', (answer) => {
      ...
      if (answer === null || answer === undefined || answer.answerSignal === undefined) { connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionError } else if (answer.accepted === false && answer.message === 'Already connected') { connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionAlreadyConnected } else if (answer.accepted === false && answer.message === "I can't accept WebPeers anymore") { this._clientIsNotAcceptingAnymoreWebPeers(client2, connection) } else if (answer.accepted === true) {
    ...
    client2.node.on('signals/server/new-answer-ice-candidate', async (iceCandidate) => {
      ...
      if (iceCandidate === null || iceCandidate === undefined) { connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionError }
    ...
    client2.node.on('signals/server/new-initiator-ice-candidate/answer', async (answer) => {
      ...
      if (answer === null || answer === undefined) { connection.status = SignalingServerRoomConnectionObject.ConnectionStatus.peerConnectionError } else if (answer.established === false && answer.message === "I can't accept WebPeers anymore") { this._clientIsNotAcceptingAnymoreWebPeers(client2, connection) }
    })
  }
