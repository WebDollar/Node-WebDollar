...
class PoolManagement {
  ...
  async initializePoolManagement (poolFee) {
    ...
    try {
      ...
      if (!answer) { throw { message: "Pool Couldn't be started" } }
    } catch (exception) {
  ...
  async startPool (forceStartPool = false) {
    if (this.poolSettings.poolURL && this.poolSettings.poolURL !== '') { return await this.setPoolStarted(true, forceStartPool) } else { console.error("Couldn't start the Pool because the poolURL is empty") }
  ...
  async setPoolStarted (value, forceStartPool = false) {
    if (this._poolStarted !== value) {
      if (value && forceStartPool) {
        ...
        if (Blockchain.ServerPoolManagement !== undefined) { await Blockchain.ServerPoolManagement.setServerPoolStarted(false) }
      }
