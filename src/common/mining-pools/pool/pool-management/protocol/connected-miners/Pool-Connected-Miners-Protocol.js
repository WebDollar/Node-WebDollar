...
class PoolConnectedMinersProtocol extends PoolProtocolList {
  ...
  async startPoolConnectedMinersProtocol () {
    for (let i = 0; i < NodesList.nodes.length; i++) { await this._subscribePoolConnectedMiners(NodesList.nodes[i].socket) }
  ...
  async _subscribePoolConnectedMiners (socket) {
    ...
    socket.node.on('mining-pool/hello-pool', async (data) => {
      try {
        ...
        let addresses = []
        if (data.addresses) {
          for (let i = 0; i < Math.min(20, data.addresses.length); i++) { addresses.push(Buffer.from(data.addresses[i], 'hex')) }
        }
        ...
        if (addresses.length > 0) { minerInstance.addresses = addresses }

        if (data.ref && (typeof data.ref === 'string' || (Buffer.isBuffer(data.ref) && data.ref.length === consts.ADDRESSES.ADDRESS.LENGTH))) { miner.referrals.setReferralLink(data.ref) }
        ...
        let suffix = ''
        if (typeof data.suffix === 'string') { suffix = '/' + data.suffix }

        // generate a message for confirming pool Owner
        let messageAddressConfirmation
        if (Blockchain.Wallet.getAddress(unencodedAddress)) { messageAddressConfirmation = new Buffer(32) }
        ...
        try {
          ...
          if (confirmation.result) {
            ...
            if (Blockchain.Wallet.getAddress(unencodedAddress) && Buffer.isBuffer(confirmation.sig) && confirmation.sig.length > 5) {
              ...
              if (ed25519.verify(confirmation.sig, messageAddressConfirmation, address.publicKey)) { socket.node.protocol.minerPool.poolOwner = true }
            }
        ...
        if (Math.random() < 0.1) { console.log('mining-pool/hello-pool' + '/answer', exception) }

        if (!socket.disconnected) { socket.node.sendRequest('mining-pool/hello-pool' + '/answer', { result: false, message: exception.message }) }
      }
    })
    ...
    socket.node.on('mining-pool/get-work', async (data) => {
      ...
      socket.node.sendRequest('mining-pool/get-work/answer' + suffix, { result: true,
        work: work,
        reward: minerInstance.miner.rewardTotal || 0,
        confirmed: minerInstance.miner.rewardConfirmedTotal || 0,
        refReward: minerInstance.miner.referrals.rewardReferralsTotal || 0,
    ...
    socket.node.on('mining-pool/get-referrals', async (data) => {
      ...
      if (data && typeof data.suffix === 'string') { suffix = '/' + data.suffix }
    ...
    socket.node.on('mining-pool/work-done', async (data) => {
      ...
      try {
        ...
        // the new reward
        socket.node.sendRequest('mining-pool/work-done/answer' + suffix, { result: true,
          newWork: newWork,
          reward: minerInstance.miner.rewardTotal || 0,
          confirmed: minerInstance.miner.rewardConfirmedTotal || 0,
          refReward: minerInstance.miner.referrals.rewardReferralsTotal || 0,
    ...
    socket.node.on('mining-pool/change-wallet-mining', (data) => {
      ...
      if (newUnencodedAddress.equals(unencodedAddress)) { throw { message: 'addresses are the same' } }
  ...
  addElement (socket, minerInstance) {
    if (!PoolProtocolList.prototype.addElement.call(this, socket)) { minerInstance.dateActivity = new Date().getTime() / 1000 }
