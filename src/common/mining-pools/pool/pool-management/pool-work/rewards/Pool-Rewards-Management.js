...
const CONFIRMATIONS_REQUIRE_OTHER_MINERS = !consts.DEBUG
...
class PoolRewardsManagement {
  ...
  async _blockchainChanged () {
    ...
    if (this._lastTimeCheckHeight > this.blockchain.blocks.length - 1) { return }
          ...
          if (block && blockInfo.height >= this.blockchain.blocks.blocksStartingPoint && this.blockchain.blocks.length >= block.height + 5 && this.blockchain.blocks.length - 50 <= block.height) {
            for (let blockInformationMinerInstance of blockInfo.blockInformationMinersInstances) {
              if (!blockInformationMinerInstance.penalty) {
                ...
                if (block.posMinerAddress && block.posMinerAddress.equals(blockInformationMinerInstance.minerAddress) && !block.data.minerAddress.equals(this.blockchain.mining.unencodedMinerAddress)) { penalty = true }

                if (block.data.minerAddress.equals(blockInformationMinerInstance.minerAddress)) { penalty = true }
    ...
    if (CONFIRMATION_METHOD === 1) {
      try {
        ...
        for (let blockInfo of this.poolData.blocksInfo) {
          if (blockInfo.block) {
            if (!firstBlock || blockInfo.block.height < firstBlock) { firstBlock = blockInfo.block.height }
          }
        }
        ...
        for (let i = this.blockchain.blocks.length - 1, n = Math.max(this.blockchain.blocks.blocksStartingPoint, firstBlock); i >= n; i--) {
          ...
          if (this.blockchain.mining.unencodedMinerAddress.equals(block.data.minerAddress)) { confirmationsPool++ } else {
    ...
    for (let i = this.poolData.blocksInfo.length - 1; i >= 0; i--) {
      ...
      if (this.blockchain.blocks.blocksStartingPoint < block.height) { // i can confirm the block by myself
        ...
        if (BufferExtended.safeCompare(block.hashChain, blockckahinBlock.hashChain)) {
          ...
          } else if (CONFIRMATION_METHOD === 2) { blockInfo.confirmations = (this.blockchain.blocks.length - block.height) }

          blockInfo.confirmationsFailsTrials = 0
        } else {
          if (this.blockchain.blocks.length > block.height + VALIDATION_BLOCK_CONFIRMATIONS_FAILS_START) { blockInfo.confirmationsFailsTrials++ }
        }
      } else { // i can not confirm the block because I am in browser and I need to use the server
        // not enough blocks
        if (block.height < this.blockchain.blocks.length + LIGHT_SERVER_POOL_VALIDATION_BLOCK_CONFIRMATIONS) { continue }

        found = await this._confirmUsingPoolServer(blockInfo)

        if (!found) { blockInfo.confirmationsFailsTrials++ }
      }

      if (!found) { blockInfo.confirmations = 0 }
    ...
    if (needSave) { await this.poolManagement.poolData.saveBlocksInformation() }
  }
  ...
  async _confirmUsingPoolServer (blockInfo) {
    ...
    try {
      ...
      for (let i = blockInfo.height - 1; i < n; i++) {
        ...
        if (!result) { throw { message: 'validation failed' } }
      }
  ...
  _createServerBlockValidation (height, blockInfoHeight) {
    ...
    if (height < this._serverBlocksDifficultyCalculation.difficultyCalculationStarts) { validationType['skip-difficulty-recalculation'] = true }

    // it's a new light fork && i have less than forkHeight
    if (blockInfoHeight < consts.BLOCKCHAIN.TIMESTAMP.VALIDATION_NO_BLOCKS) { validationType['skip-validation-timestamp'] = true }
  ...
  redistributePoolDataBlockInformation (blockInformation, index, workType) {
    if (!workType) { blockInformation.block = undefined } // cancel the block
    ...
    if (!newBlockInformation || newBlockInformation.block || newBlockInformation === blockInformation) { newBlockInformation = this.poolData.addBlockInformation() }
    ...
    for (let blockInformationMinersInstance of blockInformation.blockInformationMinersInstances) {
      ...
      if (!workType || workType === 'pow') {
        for (let height in blockInformationMinersInstance._minerInstanceTotalDifficultiesPOW) { newBlockInformationMinerInstance.adjustDifficulty({ height: height }, blockInformationMinersInstance._minerInstanceTotalDifficultiesPOW[height], true) }
      }

      if (!workType || workType === 'pos') {
        for (let height in blockInformationMinersInstance._minerInstanceTotalDifficultiesPOS) { newBlockInformationMinerInstance.adjustDifficulty({ height: height }, blockInformationMinersInstance._minerInstanceTotalDifficultiesPOS[height], true) }
      }
    ...
    if (!workType) { this.poolData.deleteBlockInformation(index) }
  }
}
