...
class PoolPayouts {
  constructor (poolManagement, poolData, blockchain) {
    ...
    StatusEvents.on('blockchain/blocks-count-changed', async (data) => {
      ...
      if (this.blockchain.blocks.length % PAYOUT_INTERVAL === 0) { await this.doPayout() }
    })
  ...
  async _doPayout () {
    ...
    for (let blockInfo of this.poolData.blocksInfo) {
      if (blockInfo.block && blockInfo.confirmed && !blockInfo.payout && !blockInfo.payoutTransaction) {
        if ((BlockchainGenesis.isPoSActivated(blockInfo.block.height) && paymentType === 'pos') || (!BlockchainGenesis.isPoSActivated(blockInfo.block.height) && paymentType === 'pow')) { blocksConfirmed.push(blockInfo) }
      }
    }
    ...
    try {
      for (let miner of this.poolData.miners) { miner.__tempRewardConfirmedOther = 0 }
      ...
      for (let blockConfirmed of blocksConfirmed) { totalSumReward += BlockchainMiningReward.getReward(blockConfirmed.block.height) * (1 - this.poolManagement.poolSettings.poolFee) }
      ...
      if (poolFork > 0) { poolForkDifferencePerBlock = Math.ceil(poolFork / blocksConfirmed.length) }

      for (let blockConfirmed of blocksConfirmed) {
        let totalDifficultyPOW = new BigNumber(0); let totalDifficultyPOS = new BigNumber(0)
        ...
        if (!totalDifficultyPOW.isEqualTo(blockConfirmed.totalDifficultyPOW)) { console.error("Total POW Difficulty doesn't match", { totalDifficultyPOW: totalDifficultyPOW, blockConfirmedDifficulty: blockConfirmed.totalDifficultyPOW }) }

        if (!totalDifficultyPOS.isEqualTo(blockConfirmed.totalDifficultyPOS)) { console.error("Total POS Difficulty doesn't match", { totalDifficultyPOS: totalDifficultyPOS, blockConfirmedDifficulty: blockConfirmed.totalDifficultyPOS }) }

        if (totalDifficultyPOS.isLessThanOrEqualTo(0) && totalDifficultyPOW.isLessThanOrEqualTo(0)) {
          if (consts.MINING_POOL.SKIP_POS_REWARDS || consts.MINING_POOL.SKIP_POW_REWARDS) { continue }
        ...
        for (let blockInformationMinerInstance of blockConfirmed.blockInformationMinersInstances) {
          ...
          if (blockInformationMinerInstance.miner.referrals.referralLinkMiner) { blockInformationMinerInstance.miner.referrals.referralLinkMiner.miner.__tempRewardConfirmedOther += blockInformationMinerInstance.rewardForReferral }
        }
      ...
      for (let toAddress of this._toAddresses) { toAddress.amount = Math.floor(toAddress.amount) }
      ...
      for (let i = this._toAddresses.length - 1; i >= 0; i--) {
        if (this._toAddresses[i].amount < consts.MINING_POOL.MINING.MINING_POOL_MINIMUM_PAYOUT) { this._removeAddressTo(this._toAddresses[i].address) }
      }

      let totalToPay = 0

      for (let toAddress of this._toAddresses) { totalToPay += toAddress.amount }
      ...
      for (let blockConfirmed of blocksConfirmed) {
        for (let blockInformationMinerInstance of blockConfirmed.blockInformationMinersInstances) {
          ...
          try {
            ...
            if (!paid) { miner.rewardConfirmedOther += miner.__tempRewardConfirmedOther }
      ...
      for (let toAddress of this._toAddresses) {
        ...
        if (i % 100 === 0) { await this.blockchain.sleep(200) }
      ...
      if (transactions.length > 0) {
        for (let blockConfirmed of blocksConfirmed) { blockConfirmed.payoutTx = transactions[0].txId }
      }
    } catch (exception) {
