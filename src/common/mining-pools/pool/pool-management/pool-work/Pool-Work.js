...
class PoolWork {
  ...
  startGarbageCollector () {
    if (!this._garbageCollectorInterval) { this._garbageCollectorInterval = setTimeout(this._garbageCollector.bind(this), 30000) }
  }
  ...
  getNextBlockForWork () {
    if (!Blockchain.synchronized) { throw { message: 'Blockchain is not yet synchronized' } }

    // still pending
    if (this.lastBlockPromise && this.lastBlockPromise.isPending()) { return this.lastBlockPromise }
    ...
    this.lastBlockPromise = Utils.makeQuerablePromise(new Promise(async (resolve) => {
      ...
      if (!this.blockchain.semaphoreProcessing.processing && (this.lastBlock.height !== this.blockchain.blocks.length || !this.lastBlock.hashPrev.equals(this.blockchain.blocks.last.hash))) {
        if (consts.DEBUG) { console.error("ERROR!!! POOL MINING LAST BLOCK WAS CHANGED. HASHPREV DOESN'T MATCH blocks.last.hash") }
      ...
      for (let i = 0; i < this._blocksList.length; i++) {
        ...
        // delete block
        {
          if (this._blocksList[i].block !== this.lastBlock && ((time - this._blocksList[i].block.timeStamp) > 200 * consts.BLOCKCHAIN.DIFFICULTY.TIME_PER_BLOCK) && this._blocksList[i].block.height < this.blockchain.blocks.length - 100) {
            ...
            for (let key in this._blocksList[i].instances) { this._blocksList[i].instances[key].workBlock = undefined }
            ...
            if (destroyed % 10 === 0) { await this.blockchain.sleep(100) }
