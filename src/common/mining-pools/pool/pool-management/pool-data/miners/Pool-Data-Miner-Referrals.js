...
class PoolDataMinerReferrals {
  ...
  destroyPoolDataMinerReferrals () {
    ...
    for (let i = 0; i < this.referees.length; i++) { this.referees[i].destroyPoolDataMinerReferral() }

    this.referees = []
  }

  setReferralLink (address) {
    if (typeof address === 'string') { address = InterfaceBlockchainAddressHelper.getUnencodedAddressFromWIF(address) }

    if (!address || !Buffer.isBuffer(address) || address.length !== consts.ADDRESSES.ADDRESS.LENGTH) { return false }

    // avoid to be the same person
    if (address.equals(this.miner.address)) { return false }
  ...
  refreshRefereeAddresses () {
    for (let i = 0; i < this.referees.length; i++) { this.referees[i].findRefereeAddress() }

    this.findReferralLinkAddress()
  }

  findReferralLinkAddress () {
    if (this.referralLinkMiner) { return this.referralLinkMiner }
  ...
  serializeReferrals () {
    ...
    if (this.referralLinkAddress) { list.push(this.referralLinkAddress) }
    ...
    if (this.referees.length > 0) {
      ...
      for (let i = 0; i < this.referees.length; i++) { list.push(this.referees[i].serializeMinerReferral()) }
  ...
  deserializeReferrals (buffer, offset) {
    ...
    if (hasReferralLink === 0x01) {
      ...
      if (!address.equals(this.miner.address)) { this.referralLinkAddress = address }
    }
    ...
    if (hasReferrals === 1) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (!referral.refereeAddress.equals(this.miner.address)) { this.referees.push(referral) }
      }
  ...
  findReferral (refereeAddress, returnPos = false) {
    for (let i = 0; i < this.referees.length; i++) {
      if (this.referees[i].refereeAddress.equals(refereeAddress)) { return returnPos ? i : this.referees[i] }
    }
  ...
  toJSON () {
    let referees = []
    for (let i = 0; i < this.referees.length; i++) { referees.push(this.referees[i].toJSON()) }
