...
class PoolDataMiner {
  ...
  destroyPoolDataMiner () {
    ...
    for (let i = 0; i < this.instances.length; i++) { this.instances[i].destroyPoolDataMinerInstance() }
  ...
  addInstance (socket, miner) {
    if (socket.node.protocol.minerPool && socket.node.protocol.minerPool.minerInstance && socket.node.protocol.minerPool.miner === miner) { return socket.node.protocol.minerPool.minerInstance }
  ...
  findInstance (socket, returnPos = false) {
    for (let i = 0; i < this.instances.length; i++) {
      if (this.instances[i].socket === socket) { return returnPos ? i : this.instances[i] }
    }
  ...
  findEmptyInstance (miner) {
    for (let i = 0; i < this.instances.length; i++) {
      if (this.instances[i].miner === miner && (!this.instances[i].socket || this.instances[i].socket.disconnected)) { return this.instances[i] }
    }
  }
  ...
  serializeMiner () {
    ...
    if (version >= 0x03) { list.push(this.referrals.serializeReferrals()) }
  ...
  deserializeMiner (buffer, offset) {
    ...
    if (version >= 0x03) { offset = this.referrals.deserializeReferrals(buffer, offset) }
  ...
  get minerHashesPerSecond () {
    ...
    for (let i = 0; i < this.instances.length; i++) { hashesPerSecond += this.instances[i].hashesPerSecond }
