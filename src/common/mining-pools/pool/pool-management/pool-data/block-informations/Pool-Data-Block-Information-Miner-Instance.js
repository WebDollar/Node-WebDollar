...
class PoolDataBlockInformationMinerInstance {
    ...
    adjustDifficulty( prevBlock, difficulty, useDeltaTime = false, calculateReward = true, blockInformationFirstMinerInstance ){
        ...
        if (!blockInformationFirstMinerInstance)
            blockInformationFirstMinerInstance = this.blockInformation.findFirstMinerInstance( this.address );


        //POS difficulty
        if (BlockchainGenesis.isPoSActivated( height )){

            if (blockInformationFirstMinerInstance.penalty)
                return;
    ...
    calculateReward( useDeltaTime = false ){
        ...
        if (useDeltaTime) {
            ...
            if (diff > 0 && this.blockInformation._timeRemaining > 0)
                if( diff + this.blockInformation._timeRemaining !== 0 )
                    ratio = new BigNumber( diff).dividedBy( diff + this.blockInformation._timeRemaining*10 );
                else
                    ratio = new BigNumber( diff);
        ...
        let reward =  rewardDifficulty.multipliedBy(ratio).multipliedBy( BlockchainMiningReward.getFinalReward( this._lastHeight)  ).multipliedBy( 1-this.poolManagement.poolSettings.poolFee).toNumber();
    ...
    deserializeBlockInformationMinerInstance(buffer, offset=0, version){
        ...
        if (version === 0x02){
            ...
            for (let i=0; i  < powLength; i++){
                ...
                if (difficulty.isGreaterThan(0) && this.minerInstance )
