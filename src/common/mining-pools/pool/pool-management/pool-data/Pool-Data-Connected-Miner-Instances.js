...
class PoolDataConnectedMinerInstances extends PoolProtocolList {
  ...
  startPoolDataConnectedMinerInstances () {
    if (!this._deleteUnresponsiveMinersInterval) { this._deleteUnresponsiveMinersInterval = setTimeout(this._deleteUnresponsiveMiners.bind(this), 20000) }
  }
  ...
  async _deleteUnresponsiveMiners () {
    ...
    try {
      ...
      for (let i = this.connectedMinerInstances.length - 1; i >= 0; i--) {
        if (!this.connectedMinerInstances[i] || !this.connectedMinerInstances[i].socket || this.connectedMinerInstances[i].socket.disconnected) { this.connectedMinerInstances.splice(i, 1) } else
        if (time - this.connectedMinerInstances[i].dateActivity > 480) { // 8 minutes
          try {
            ...
            if (this.connectedMinerInstances[i] && socket === this.connectedMinerInstances[i].socket) { this.connectedMinerInstances.splice(i, 1) }
  ...
  findElement (socket) {
    for (let i = 0; i < this.list.length; i++) {
      if (this.list[i].socket === socket || this.list[i] === socket) { return i }
    }
  ...
  deleteElement (socket) {
    for (let i = this.list.length - 1; i >= 0; i--) {
      if (this.list[i].socket === socket || this.list[i] === socket) { this.list.splice(i, 1) }
    }
  }
}
