...
class ServerPoolManagement {
  ...
  async initializeServerPoolManagement (serverPoolFee) {
    if (await this.serverPoolData.loadServerPoolsList() === false) { throw { message: 'loadPoolsList failed' } }

    if (await this.serverPoolSettings.initializeServerPoolSettings() === false) { throw { message: "loadServer didn't work" } }

    if (serverPoolFee !== undefined && typeof serverPoolFee === 'number') { await this.serverPoolSettings.setServerPoolFee(serverPoolFee) }
  ...
  async setServerPoolStarted (value, forceStartServerPool) {
    if (this._serverPoolStarted !== value) {
      ...
      if (value) {
        if (this.blockchain !== undefined && this.blockchain.prover !== undefined) { this.blockchain.prover.proofActivated = false }
        ...
        if (this.blockchain !== undefined && this.blockchain.prover !== undefined) { this.blockchain.prover.proofActivated = true }
