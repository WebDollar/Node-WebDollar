...
class MinerPoolMining extends InheritedPoolMining {
  constructor (minerPoolManagement) {
    ...
    NodesList.emitter.on('nodes-list/disconnected', (nodesListObject) => {
      if (nodesListObject.socket === this._miningWork.poolSocket) { this._miningWork.poolSocket = null }
    })
  ...
  _startMinerPoolMining () {
    if (!this._checkForWorkInterval) { this._checkForWorkInterval = this._checkForWorkIntervalCallback() }
  ...
  async _setAddress (newAddress, save, skipChangingAddress = false) {
    ...
    if (Blockchain.Wallet.getAddress(this._minerAddress) === null) {
      ...
      if (await AdvancedMessages.confirm('You are mining on a different address in this pool. Do you want to change the pool mining address')) { await this.minerPoolManagement.minerPoolProtocol.changeWalletMining() }
    } else { await this.minerPoolManagement.minerPoolProtocol.changeWalletMining(undefined, this._minerAddress, oldMinerAddress) }
  }
  ...
  updatePoolMiningWork (work, poolSocket) {
    ...
    if (work.b && work.b.length === Blockchain.Wallet.addresses.length) {
      for (let i = 0; i < work.b.length; i++) { this._miningBalances[ Blockchain.Wallet.addresses[i].unencodedAddress.toString('hex') ] = work.b[i] }
    }
    ...
    // required data
    if (BlockchainGenesis.isPoSActivated(work.h)) { block.posMinerAddress = Blockchain.Mining.unencodedMinerAddress }
  ...
  async mineNextBlock (suspend) {
    while (this.started && !global.TERMINATED) {
      if (!this._miningWork.block || this._miningWork.resolved) { await Blockchain.blockchain.sleep(5) } else {
        try {
          ...
          this._isBeingMining = new Promise(async (resolve) => {
            try {
              ...
              if (!this.resetForced) { this._miningWork.resolved = true } else this.resetForced = false
