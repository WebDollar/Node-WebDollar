...
class MinerPoolProtocol extends PoolProtocolList {
  ...
  async _startMinerProtocol () {
    ...
    for (let i = 0; i < NodesList.nodes.length; i++) { await this._subscribeMiner(NodesList.nodes[i]) }
  ...
  _unsubscribeMiner (nodesListObject) {
    ...
    if (socket.node.protocol.nodeConsensusType === NODE_CONSENSUS_TYPE.NODE_CONSENSUS_POOL) { StatusEvents.emit('miner-pool/servers-connections', { message: 'Server Removed' }) }
  }
  ...
  async _sendPoolHello (socket) {
    try {
      ...
      let addresses = []
      for (let i = 0; i < Blockchain.Wallet.addresses.length; i++) { addresses.push(Blockchain.Wallet.addresses[i].unencodedAddress.toString('hex')) }
      ...
      try {
        ...
        if (answer.work) { await this._validateRequestWork(answer.work, socket) }
  ...
  async _connectionEstablishedWithPool (socket) {
    ...
    if (this.minerPoolManagement.minerPoolMining._isBeingMining) { await this.minerPoolManagement.minerPoolMining._isBeingMining }
    ...
    if (this.minerPoolManagement.blockchain.agent.status === AGENT_STATUS.AGENT_STATUS_NOT_SYNCHRONIZED) { this.minerPoolManagement.blockchain.agent.status = AGENT_STATUS.AGENT_STATUS_SYNCHRONIZED }
  ...
  async pushWork (miningAnswer, poolSocket) {
    try {
      ...
      if (miningAnswer.pos) { Log.info('timestamp ' + miningAnswer.pos.timestamp + '   ' + 'balance ' + (Blockchain.AccountantTree.getBalance(Blockchain.blockchain.mining.minerAddress) / WebDollarCoins.WEBD), Log.LOG_TYPE.POOLS) }
  ...
  async changeWalletMining (poolSocket, newAddress, oldAddress) {
    ...
    try {
      ...
      if (!newAddress) { newAddress = Blockchain.Wallet.addresses[0].address }
      ...
      if (oldAddress) { await this.minerPoolManagement.minerPoolMining._setAddress(oldAddress.address, false, true) }
