...
class MinerPoolSettings {
  constructor (minerPoolManagement, databaseName) {
    ...
    this._db = new InterfaceSatoshminDB(databaseName || consts.DATABASE_NAMES.MINER_POOL_DATABASE)
  ...
  generatePoolURLReferral () {
    ...
    if (url.indexOf('/r/', url) >= 0) { url = url.substr(0, url.indexOf('/r/', url)) }
  ...
  async setPoolURL (newValue, skipSaving = false) {
    ...
    if (!skipSaving) { if (await this._db.save('minerPool_poolURL', this._poolURL) === false) throw { message: "PoolURL couldn't be saved" } }
  ...
  async _setPoolServers (newValue) {
    ...
    if (this.minerPoolManagement.minerPoolStarted) { await this.minerPoolManagement.minerPoolProtocol.insertServersListWaitlist(this.poolServers) }
  }
  ...
  async addPoolList (url, data, save = true) {
    if (!data) { data = PoolsUtils.extractPoolURL(url) }
  ...
  async setMinerPoolActivated (newValue, skipSaving = false, useActivation = true) {
    ...
    if (!skipSaving) { if (await this._db.save('minerPool_activated', this._minerPoolActivated ? 'true' : 'false') === false) throw { message: "minerPoolActivated couldn't be saved" } }
    ...
    if (useActivation) { await this.minerPoolManagement.setMinerPoolStarted(newValue, true) }
  }
  ...
  async _replacePoolURL (url = '') {
    ...
    if (url.indexOf('WMP/0.02/bdda9527a040d9063e22e1dccc4d860b84227ff73232f5c418054112114a6ea4/https:$$pool.webdollarminingpool.com:443') >= 0) { url = url.replace('WMP/0.02/bdda9527a040d9063e22e1dccc4d860b84227ff73232f5c418054112114a6ea4/https:$$pool.webdollarminingpool.com:443', 'WMP/0.02/c01f57930c27e78e434de1243ae02b98e56d6cd3df42d136be1a1c0a0a9a8624/https:$$server.webdollarminingpool.com:443') }
