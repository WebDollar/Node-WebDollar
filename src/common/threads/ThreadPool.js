...
class ThreadPool {
    ...
    this._pool
      ...
      .on('finished', () => {
        ...
        if (this._updateNumWorkers === true) {
          ...
          for (let i = 0; i < this._queue.length; ++i) { this._pool.run(this._queue[i].taskCallback).send(this._queue[i].inputData) }
  ...
  addTask (taskCallback, inputData) {
    if (this._updateNumWorkers === false) { this._pool.run(taskCallback).send(inputData) } else { this._queue.push({ taskCallback: taskCallback, inputData: inputData }) }
  }
}
