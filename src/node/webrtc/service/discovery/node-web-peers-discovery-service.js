...
class NodeWebPeersDiscoveryService {
  ...
  startDiscovery () {
    ...
    for (let i = 0; i < NodesList.nodes.length; i++) { this._newSocketRegisterAcceptWebPeers(NodesList.nodes[i]) }
  }
  ...
  _newSocketRegisterAcceptWebPeers (nodesListObject) {
    ...
    if ([ NODES_CONSENSUS_TYPE.NODE_CONSENSUS_SERVER_FOR_MINER].indexOf(nodesListObject.nodeConsensusType) >= 0) { return }

    // client Signaling for WebRTC
    if ([ CONNECTIONS_TYPE.CONNECTION_CLIENT_SOCKET, CONNECTIONS_TYPE.CONNECTION_WEBRTC].indexOf(nodesListObject.connectionType) >= 0) { nodesListObject.socket.node.protocol.signaling.client.initializeSignalingClientService() }
  }
}
