...
class NodesWaitlistObject {
  constructor (sckAddresses, nodeType, nodeConsensusType, level, backedBy, connected = false, socket = null) {
    ...
    if (backedBy === 'fallback') { this.isFallback = true } else { this.isFallback = false }
  ...
  checkLastTimeChecked (timeTryReconnectAgain) {
    ...
    if ((time - this.lastTimeChecked) >= timeTryReconnectAgain + this.errorTrials * (3000 + Math.random() * 8000)) { return true }
  ...
  socketErrorConnected () {
    ...
    if (this.isFallback === true) {
      if (process.env.BROWSER) { this.errorTrials = Math.min(this.errorTrials, 3 + Math.floor(Math.random() * 2)) } else { this.errorTrials = Math.min(this.errorTrials, 4 + Math.floor(Math.random() * 5)) }
    }
  }
  ...
  toString () {
    let text = ''
    for (let i = 0; i < this.sckAddresses.length; i++) { text += this.sckAddresses[i].toString() + '   ' }
  ...
  pushBackedBy (sckAddress, connected) {
    if (sckAddress && typeof sckAddress === 'object' && sckAddress.node && sckAddress.node.sckAddress) { sckAddress = sckAddress.node.sckAddress }
    ...
    if (this.nodeType === NODE_TYPE.NODE_WEB_PEER) { max = MAX_NUMBER_OF_BACKED_BY_LIGHT_NODE } else if (this.nodeType === NODE_TYPE.NODE_TERMINAL) { max = MAX_NUMBER_OF_BACKED_BY_FULL_NODE }
  ...
  removeBackedBy (sckAddress) {
    for (let i = 0; i < this.backedBy.length; i++) {
      if (this.backedBy[i].sckAddress === sckAddress) {
        if (this.backedBy[i].connected) { this.backedByConnected-- }
  ...
  findBackedBy (sckAddress) {
    for (let i = 0; i < this.backedBy.length; i++) {
      if (this.backedBy[i].sckAddress === sckAddress) { return true }
    }
