...
class NodesStats {
  ...
  _printStats () {
    ...
    try {
      if (Blockchain.blockchain.mining.minerAddress && Buffer.isBuffer(Blockchain.blockchain.mining.unencodedMinerAddress)) { console.info('Exists: ', Blockchain.Wallet.getAddress({ unencodedAddress: Blockchain.blockchain.mining.unencodedMinerAddress }) !== null ? 'YES' : 'NO') }
    } catch (err) {
    ...
    for (let i = 0; i < Math.min(clients.length, 50); i++) { string1 += '(' + clients[i].socket.node.sckAddress.toString() + ')   ' }
    if (clients.length > 50) string1 += '.........'

    let string2 = ''
    let server = NodesList.getNodesByConnectionType(CONNECTIONS_TYPE.CONNECTION_SERVER_SOCKET)
    for (let i = 0; i < Math.min(server.length, 50); i++) { string2 += '(' + server[i].socket.node.sckAddress.toString() + ')   ' }
