...
class NodesList {
  ...
  isConsensus (blockchainHeight) {
    ...
    if (this.nodes.length > 0) {
      for (let i = 0; i < this.nodes.length; i++) {
        if (this.nodes[i].socket.node.protocol.blocks && this.nodes[i].socket.node.protocol.blocks > 0) {
          if (!blocksList[this.nodes[i].socket.node.protocol.blocks]) { blocksList[this.nodes[i].socket.node.protocol.blocks] = 1 } else { blocksList[this.nodes[i].socket.node.protocol.blocks]++ }
        }
      }
    } else { return false }
    ...
    if (blockchainHeight > 5 && blockchainHeight > this.consensusBlock - 4) { return true } else { return false }
  }
  ...
  searchNodeSocketByAddress (sckAddress, connectionType = 'all', validationDoubleConnectionsTypes) {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      if ((this.nodes[i].connectionType === connectionType || connectionType === 'all') && (this.nodes[i].socket.node.sckAddress.matchAddress(sckAddress, validationDoubleConnectionsTypes))) { return this.nodes[i] }
    }
  ...
  countNodeSocketByAddress (sckAddress, connectionType = 'all') {
    ...
    let countUUIDs = 0; let countIPs = 0
    for (let i = 0; i < this.nodes.length; i++) {
      if (this.nodes[i].connectionType === connectionType || connectionType === 'all') {
        if (this.nodes[i].socket.node.sckAddress.uuid === sckAddress.uuid) { countUUIDs++ } else
        if (this.nodes[i].socket.node.sckAddress.address === sckAddress.address) { countIPs++ }
      }
    }
  ...
  async disconnectSocket (socket, connectionType = 'all') {
    ...
    for (let i = this.nodes.length - 1; i >= 0; i--) {
      if ((this.nodes[i].connectionType === connectionType || connectionType === 'all') &&
        ...
        if (Math.random() < 0.3) { console.error('deleting client socket ' + i + ' ' + socket.node.sckAddress.toString()) }
  ...
  getNodesByConnectionType (connectionType = 'all', fallback) {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      if (Array.isArray(connectionType)) { // in case type is an Array
        if (connectionType.indexOf(this.nodes[i].socket.node.protocol.connectionType) >= 0) { list.push(this.nodes[i]) }
      } else
      // in case type is just a simple string
      if (connectionType === this.nodes[i].socket.node.protocol.connectionType || connectionType === 'all') { list.push(this.nodes[i]) }
    }
  ...
  getNodesByType (type = 'all') {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      if (Array.isArray(type)) { // in case type is an Array
        if (type.indexOf(this.nodes[i].socket.node.protocol.nodeType) >= 0) { list.push(this.nodes[i]) }
      } else
      // in case type is just a simple string
      if (type === this.nodes[i].socket.node.protocol.nodeType || type === 'all') { list.push(this.nodes[i]) }
    }
  ...
  disconnectAllNodes (connectionType = CONNECTION_TYPE.CONNECTION_CLIENT_SOCKET) {
    for (let i = this.nodes.length - 1; i >= 0; i--) {
      if (this.nodes[i].socket.node.protocol.connectionType === connectionType || connectionType === 'all') { this.nodes[i].socket.disconnect() }
    }
  }

  disconnectAllNodesByConsensusType (nodeConsensusType) {
    for (let i = this.nodes.length - 1; i >= 0; i--) {
      if (this.nodes[i].socket.node.protocol.nodeConsensusType === nodeConsensusType) { this.nodes[i].socket.disconnect() }
    }
  }

  disconnectFromFallbacks () {
    for (let i = this.nodes.length - 1; i >= 0; i--) {
      if (this.nodes[i].isFallback) { this.nodes.disconnect() }
    }
  }
  ...
  countFallbacks () {
    ...
    for (let i = this.nodes.length - 1; i >= 0; i--) {
      if (this.nodes[i].isFallback) { count++ }
    }
  ...
  async recalculateSocketsLatency () {
    try {
      for (let i = 0; i < this.nodes.length; i++) { await this.nodes[i].socket.node.protocol.calculateLatency() }
    } catch (exception) {
