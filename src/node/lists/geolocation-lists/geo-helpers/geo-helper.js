...
class GeoHelper {
  async getLocationFromAddress (address, skipSocketAddress) {
    ...
    if (!skipSocketAddress) {
      ...
      if (sckAddress._geoLocation.isFulfilled()) { return sckAddress._geoLocation }
    }
    ...
    try {
      ...
      if (data) {
        ...
        if (address === '') { if (data.query) result.address = address || data.query }

        if (result.countryCode) { result.continent = getContinentFromCountry(result.countryCode) }
        ...
        if (!skipSocketAddress) { sckAddress._geoLocationResolver(result) }
      }
    } catch (Exception) {
