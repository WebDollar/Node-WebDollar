...
class GeoLocationLists {
  ...
  includeAddress (sckAddress, port) {
    ...
    if (this._addressesResolved[address]) { return this._addressesResolved[address] }
  ...
  printGeoLocationContinentsLists () {
    for (let continent in this.geoLocationContinentsLists) {
      ...
      for (let i = 0; i < this.geoLocationContinentsLists[continent].length; i++) { listString += this.geoLocationContinentsLists[continent][i].toString() + '   ,   ' }
    }
  }
}
