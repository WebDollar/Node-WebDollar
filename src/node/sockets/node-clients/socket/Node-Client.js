...
class NodeClient {
  ...
  connectTo (address, port, level, SSL, waitlist) {
    ...
    return new Promise(async (resolve) => {
      ...
      try {
        ...
        if (process.env.BROWSER && consts.SETTINGS.NODE.SSL) { SSL = true }
        ...
        if (Blockchain.MinerPoolManagement.minerPoolStarted && waitlist.nodeConsensusType !== NODES_CONSENSUS_TYPE.NODE_CONSENSUS_SERVER) { throw { message: 'You switched to pool' } }
  ...
  async initializeSocket (validationDoubleConnectionsTypes, waitlist) {
    ...
    if (await NodesList.registerUniqueSocket(this.socket, CONNECTIONS_TYPE.CONNECTION_CLIENT_SOCKET, this.socket.node.protocol.nodeType, waitlist.nodeConsensusType, validationDoubleConnectionsTypes) === false) { return false }
