...
class NodeDiscoveryService {
  constructor () {
    ...
    for (let i = 0; i < this.fallbackLists.length; i++) { this.fallbackLists[i] = new FallBackObject(this.fallbackLists[i]) }
  }
    ...
    if (NodesList.nodes !== null && NodesList.nodes.length < 5) {
      for (let i = 0; i < this.fallbackLists.length; i++) {
        if (this.fallbackLists[i].checked === false && this.fallbackLists[i].checkLastTimeChecked(consts.SETTINGS.PARAMS.FALLBACK_INTERVAL)) {
          ...
          if (answer !== null) { return await this.processFallbackNodes(answer) }
        }
      }
    }

    if (setTimeOut === true) { setTimeout(() => { return this._discoverFallbackNodes(setTimeOut) }, 3000) }
  }
        ...
        if ((data.hasOwnProperty('protocol')) && (data['protocol'] === consts.SETTINGS.NODE.PROTOCOL)) {
          ...
          if (Array.isArray(nodes)) {
            ...
            for (let i = 0; i < nodes.length; i++) {
              ...
              while (marked[pos] !== undefined) { pos = Math.floor(Math.random() * nodes.length) }

              marked[pos] = true

              let nodeAddress = ''; let nodePort
              let nodeType = NODE_TYPE.NODE_TERMINAL
