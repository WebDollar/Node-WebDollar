...
class TestsHelper {
  makeId (count, randomLengths, textPossible = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789') {
    ...
    if (randomLengths) { count = randomLengths + Math.floor(Math.random() * (count + randomLengths)) }
  ...
  makeDigitId (count, isNonDecimal, negative = false) {
    ...
    for (; i < count; i++) { text += digits.charAt(Math.floor(Math.random() * 10)) }

    if (negative) {
      if (Math.floor(Math.random() * 2) === 0) { negative = false }
    }
  ...
  makeRandomNumber (biggestNumber, negative = false) {
    ...
    if (negative) {
      if (Math.floor(Math.random()) % 2 === 0) { return -number }
    }
  ...
  makePermutations (radixTestingArray) {
    ...
    for (let i = 0; i < used.length; ++i) { used[i] = 0 }
  ...
  generateAddresses (count, onlyAddress = true) {
    ...
    while (list.length < count) {
      ...
      if (!found) {
        if (onlyAddress) { list.push(address.address) } else { list.push(address) }
      }
    }
