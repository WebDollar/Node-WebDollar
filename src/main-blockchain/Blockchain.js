...
let ServerPoolManagement

if (!process.env.BROWSER) { ServerPoolManagement = require('common/mining-pools/server-pool/Server-Pool-Management').default }
...
class Blockchain {
  ...
  async initializeBlockchain (beforeBlockchainLoadCallback, afterBlockchainLoadCallback, afterSynchronizationCallback, synchronize = true) {
    ...
    if (typeof beforeBlockchainLoadCallback === 'function') { await beforeBlockchainLoadCallback() }
    ...
    if (typeof afterBlockchainLoadCallback === 'function') { await afterBlockchainLoadCallback() }

    // loading the blockchain
    if (synchronize) {
      await this.Agent.initializeStartAgentOnce()

      if (this.Agent.consensus) { await this.synchronizeBlockchain(true) } // it tries synchronizing multiple times
      else {
    ...
    if (typeof afterSynchronizationCallback === 'function') { await afterSynchronizationCallback() }

    this.loaded = true
  }

  async startMining () {
    if (process.env.START_MINING || this._startMiningNextTimeSynchronized) { return this.Mining.startMining() }
  }
  ...
  async synchronizeBlockchain (firstTime, synchronizeComplete = false) {
    ...
    while (!this.synchronized) {
      ...
      if (resultAgentStarted.result) {
        ...
        if (firstTime) { StatusEvents.emit('blockchain/status', { message: 'Error Synchronizing' }) } else { StatusEvents.emit('blockchain/status', { message: 'No Internet Access' }) }

        if (NodesList.nodes.length === 0) { NodesWaitlist.resetWaitlist(NODE_TYPE.NODE_WEB_PEER) }
  ...
  set startMiningNextTimeSynchronized (newValue) {
    ...
    if (newValue && this.synchronized) { this.startMining() }
  }
  ...
  async createMiningPools () {
    if (!this.PoolManagement) { this.PoolManagement = new PoolManagement(this.blockchain, this.Wallet) }

    if (!this.MinerPoolManagement) { this.MinerPoolManagement = new MinerPoolManagement(this.blockchain) }

    if (ServerPoolManagement && !this.ServerPoolManagement) { this.ServerPoolManagement = new ServerPoolManagement() }
  ...
  async _initializeMiningPools () {
    let pool = false; let minerPool = false; let serverPool = false
    ...
    try {
      if (this.ServerPoolManagement !== undefined) { serverPool = await this.ServerPoolManagement.initializeServerPoolManagement() }
    } catch (exception) {
  ...
  async _startMiningPools () {
    if (this.MinerPoolManagement.minerPoolSettings.minerPoolActivated) { await this.MinerPoolManagement.setMinerPoolStarted(this.MinerPoolManagement.minerPoolSettings.minerPoolActivated, true) }

    if (this.PoolManagement.poolSettings.poolActivated) { await this.PoolManagement.setPoolStarted(this.PoolManagement.poolSettings.poolActivated, true) }

    if (this.ServerPoolManagement !== undefined && this.ServerPoolManagement.serverPoolSettings.serverPoolActivated) { await this.ServerPoolManagement.setServerPoolStarted(this.ServerPoolManagement.serverPoolSettings.serverPoolActivated, true) }
  }
  ...
  get versionCompatibility () {
    if (consts.SETTINGS.NODE.VERSION_COMPATIBILITY_UPDATE) { return consts.SETTINGS.NODE.VERSION_COMPATIBILITY_UPDATE } else { return consts.SETTINGS.NODE.VERSION_COMPATIBILITY }
  }
}
